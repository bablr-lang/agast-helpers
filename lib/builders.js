...
export const buildFacadeProperty = (reference, binding = null, node = null, shift = null) => {
  if (node?.binding) throw new Error();
  if (!reference.flags) throw new Error();
...
export const buildReference = (
  ...
  let { expression, intrinsic } = flags;

  expression = !!expression;
  intrinsic = !!intrinsic;

  return freeze({ type, name, isArray, flags: freeze({ expression, intrinsic }) });
};
...
export const buildBinding = (segments = []) => {
  if (!isArray(segments)) throw new Error();
  return freeze({ segments });
};

export const buildBindingTag = (segments = []) => {
  if (!isArray(segments)) throw new Error();
  return freeze({ type: BindingTag, value: freeze({ segments }) });
};
...
export const getFlagsWithGap = (flags, hasGap = true) => {
  ...
  if (!gapFlags) {
    gapFlags = { ...flags, intrinsic: !hasGap };
...
export const expressionReferenceFlags = freeze({
  expression: true,
  intrinsic: true, // TODO is this right?
});
