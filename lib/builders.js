...
export const buildProperty = (tags, shift) => {
  ...
  if (
    !(
      (isArray(ref) && ref[0] === 0) ||
      (isString(ref) && [ReferenceTag, ShiftTag].includes(parseTagType(ref)))
    )
  )
    throw new Error();
  ...
  return freezeRecord({
    tags,
    reference: shift ? null : (!isArray(ref) && parseTag(ref).value) || buildReference(),
...
export const parseBase64 = (input) => {
  ...
  while (inRange(chr, 'a', 'z') || inRange(chr, 'A', 'Z') || '=+/'.includes(chr)) {
    content += chr;
    chr = str[++p.idx];
  }
