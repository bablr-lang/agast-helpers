...
export const buildSpan = (name, guard = null, props = {}) => {
  ...
  return deepFreezeRecord({ name, guard, props });
};

export const buildSpanEntry = (name, guard = null, props = {}) => {
  return deepFreezeRecord([name, buildSpan(name, guard, props)]);
};
...
export const buildProperty = (tags, shift) => {
  ...
  return deepFreezeRecord({
    ...
    bindings: bindings ? [...BList.traverse(bindings)].map((tag) => parseTag(tag).value) : [],
    node: node || null,
...
export const buildPathFrame = (property, parentIndex = null, isGap = false) => {
  return deepFreezeRecord({ property, parentIndex, isGap });
};
...
export const buildReference = (type = null, name = null, flags = referenceFlags) => {
  ...
  return deepFreezeRecord({ type: type_, name, flags: { array, expression, intrinsic, hasGap } });
};
...
export const buildBinding = (segments = freezeRecord([])) => {
  // TODO relax this restriction
  if (!isArray(segments) || !segments.length) throw new Error();
  if (Array.prototype.includes.call(segments, undefined)) throw new Error();

  return deepFreezeRecord({
    segments: Array.prototype.map.call(segments, (segment) =>
      isString(segment) ? { type: null, name: segment } : segment,
    ),
  });
};

export const buildBindingTag = (segments = freezeRecord([])) => {
  return buildTag(BindingTag, buildBinding(segments));
};
...
export const buildDoctype = (version = 0, attributes = freezeRecord({})) => {
  ...
  return deepFreezeRecord({ doctype: 'cstml', version, attributes });
};

export const buildTag = (type, value) => {
  return deepFreezeRecord({ type, value });
};
...
export const buildOpenNode = (
  ...
  if (!isDeepRecord(attributes)) throw new Error();
  if (!isDeepRecord(flags)) throw new Error();
...
export const buildAttributeDefinition = (key, value) => {
  ...
  return deepFreezeRecord({ path: key, value });
};
...
export const parseUnsignedInteger = (p) => {
  let { str } = buildParser(p);
  let chr = str[p.idx];
  let digits = [];
...
export const parseBindingSegment = (p) => {
  ...
  while (sep) {
    if (chr === '.' && str[p.idx + 1] === '.') {
      p.idx += 2;
      segments.push({ type: '..', name: null });
    } else {
      segments.push({ type: null, name: parseIdentifier(p) });
      chr = str[p.idx];
    }
...
export const isValidBinding = (value) => {
  ...
  for (let segment of arrayValues(value.segments)) {
...
export const isValidDoctype = (value) => {
  ...
  if (!isDeepRecord(value.attributes)) return false;
...
export const isValidAttributeDefinition = (value) => {
  ...
  if (!isDeepRecord(value.value)) return false;
...
export const isValidProperty = (value) => {
  ...
  for (let binding of arrayValues(value.bindings)) if (!isValidBinding(binding)) throw new Error();
