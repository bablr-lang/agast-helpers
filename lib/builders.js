import { hasOwn, isObject, isSymbol, deepFreeze } from './object.js';
...
import { symbolName } from './path.js';
...
export const buildProperty = (tags, shift) => {
  ...
  if (tags[1][1] && !Tags.isNode(tags[1][1])) throw new Error();
  ...
  return freeze({
    ...
    bindings: tags[1][1] ? freeze([...Tags.traverse(tags[1][1])].map((tag) => tag.value)) : [],
    node: tags[1][2] || null,
...
export const buildChild = (type, value) => {
  ...
  if (!Tags.isValidTag(child)) throw new Error();
  return child;
};
