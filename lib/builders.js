...
export const buildReference = (type = null, name = null, flags = defaultReferenceFlags) => {
  ...
  if (type && ['_', '#'].includes(type) && (flags.intrinsic || flags.hasGap)) throw new Error();
...
export const buildIdTag = (hash) => {
  if (!isString(hash)) throw new Error();

  return buildTag(IdTag, freezeRecord({ hash }));
};

export const buildSumsTag = (refsSums, hash, lineBreaks, gaps) => {
  if (!isArray(refsSums)) throw new Error();
  if (!isString(hash)) throw new Error();
  if (!Number.isFinite(lineBreaks)) throw new Error();
  if (!Number.isFinite(gaps)) throw new Error();

  return buildTag(SumsTag, freezeRecord([freezeRecord(refsSums), hash, lineBreaks, gaps]));
};
...
export const defaultNodeFlags = freezeRecord({ token: false, hasGap: false });
