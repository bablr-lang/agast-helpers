...
export const streamFromTree = (rootNode, options = {}) => {
  ...
  if (options.unshift) {
    return __streamFromTreeUnshifted(rootNode_);
  } else {
    return __streamFromTree(rootNode_, options);
  }
};

function* __streamFromTreeUnshifted(rootNode) {
  let stack = [];
  let frame = { node: rootNode, iter: sumtree.traverse(rootNode.children)[Symbol.iterator]() };

  try {
    let step = frame.iter.next();
    while (frame) {
      while (step && !step.done) {
        ...
        if (child.type === Property) {
          ...
          frame = {
            node,
            iter: sumtree.traverse(node.children)[Symbol.iterator](),
          };
        } else {
          yield child;
        }
        step = frame.iter.next();
      }
      frame = stack.pop();
      step = frame && frame.iter.next();
    }
  } catch (e) {
    while (frame) {
      frame.iter.return();
...
export const addProperty = (node, property) => {
  ...
  if (!property.binding) throw new Error();
...
export const shiftProperty = (node, property) => {
  ...
  let index = stackSize === 1 ? 1 : stackSize + 1;
