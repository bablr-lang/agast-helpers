...
export const buildStubNode = (tag) => {
  ...
  return freeze({
    flags: nodeFlags,
    type: null,
    children: Children.fromValues([tag]),
    attributes: freeze({}),
    leftStack: null,
    rightStack: null,
  });
...
export const createNode = (openTag) => {
  ...
  if (!openTag || openTag.type === GapTag || openTag.type === NullTag) {
    return {
      ...
      children: btree.fromValues([]),
      attributes: openTag?.attributes || {},
      leftStack: btree.fromValues([gapProperty]),
      rightStack: btree.fromValues([gapProperty]),
    };
  } else {
...
export const shiftProperty = (node, property) => {
  ...
  let shiftStack = get(property.reference.name, node).leftStack || btree.fromValues([]);
