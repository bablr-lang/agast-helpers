...
export const buildToken = (type, value, attributes = {}) => {
  return treeFromStreamSync([
    buildOpenNodeTag(tokenFlags, type, null, attributes),
...
export const mergeReferences = (outer, inner) => {
  let { type, name, isArray, index, flags: { expression, intrinsic, hasGap } = {} } = outer;
  ...
  return buildReferenceTag(type, name, isArray, { expression, intrinsic, hasGap }, index).value;
};
...
function* __vcsStreamFromTree(rootNode) {
  ...
  outer: while (node) {
    ...
    if (isArray(child)) {
      ...
      yield buildOpenNodeTag(fragmentFlags);
