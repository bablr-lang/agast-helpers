...
export const mergeReferences = (outer, inner) => {
  let { type, name, isArray, index, flags: { expression, intrinsic } = {} } = outer;
  ...
  return buildReferenceTag(type, name, isArray, { expression, intrinsic }, index).value;
};
...
export const printSource = (tree) => {
  if (tree && !isPlainObject(tree)) throw new Error('bad tree');
