...
export const getCooked = (cookable) => {
  ...
  for (let tag of sumtree.traverse(children)) {
    switch (tag.type) {
      case ReferenceTag: {
        let { type } = tag.value;
...
export const addToProperties = (properties, property) => {
  ...
  if (flags.expression && !isInitializer) {
    let shiftStack;
    let firstProp = value && !Array.isArray(value) ? getFirstNodeProperty(value) : null;
    if (firstProp && firstProp.reference.flags.expression) {
      // if property.node has an expression stack in the leftmost slot, use that as our base stack
      shiftStack = firstProp.node;
    } else {
      // otherwise put in a gap and start a new stack
      shiftStack = btree.fromValues([buildProperty(reference, null, buildStubNode(buildGapTag()))]);
    }
...
export const shiftToProperties = (properties, property) => {
  ...
  let firstProp = getFirstNodeProperty(property.node);

  let shiftStack =
    (isArray ? btree.getAt(-1, properties[name].node) : properties[name])?.node ||
    (firstProp && firstProp.reference.flags.expression)
      ? property.node.properties[firstProp.reference.name].node
      : btree.fromValues([buildProperty(reference, null, buildStubNode(buildGapTag()))]);

  shiftStack = btree.push(shiftStack, property);

  let stackProperty = buildProperty(reference, null, shiftStack);

  if (isArray) {
    properties[name] = buildProperty(
      reference,
      null,
      btree.replaceAt(-1, properties[name].node, stackProperty),
    );
  } else {
    properties[name] = stackProperty;
  }
  return properties;
};
...
export const addProperty = (node, property) => {
  ...
  let back1 = sumtree.getAt(-1, node.children);
  let back2 = sumtree.getAt(-2, node.children);
  ...
  if (back1.type === BindingTag) {
    ...
    referenceTag = back2;
    foundRef = foundBinding = true;

    if ((bindingTag?.value ?? null) !== property.binding) throw new Error();
  ...
  if (isInitializer) {
    ...
    if (!foundRef) {
      node.children = sumtree.push(node.children, referenceTag);
    }
    node.children = sumtree.push(node.children, buildInitializerTag(isArrayInitializer));
  ...
  if (!foundBinding) {
    node.children = sumtree.push(node.children, bindingTag);
  }
  node.children = sumtree.push(node.children, buildChild(Property, property));
...
export const shiftProperty = (node, property) => {
  ...
  let existingBinding = getProperties(property.reference.name, node.properties);
  let stackSize = existingBinding ? btree.getSize(existingBinding.node) : 0;

  shiftToProperties(node.properties, property);

  let index = stackSize + 1;
  let height = index + btree.getSize(getFirstNode(property.node));

  let refOrShiftTag = stackSize ? buildShiftTag(index, height) : referenceTag;

  if (!foundRef) {
    node.children = sumtree.push(node.children, refOrShiftTag);
  }
  if (!foundBinding) {
    node.children = sumtree.push(node.children, bindingTag);
  }
