      ...
      switch (tag.type) {
        case Property: {
          ...
          if (replaceWithGap) {
            ...
            node = Tags.getValues(child.value.node.tags);

            depth++;
            i = 0;
            agastNode = child.value.node;
            nodeShifted = shifted;
...
export const addProperty = (node, property) => {
  ...
  let back1 = Tags.getAt(-1, node.tags);
  let back2 = Tags.getAt(-2, node.tags);
  ...
  if (isInitializer) {
    if (!foundShift) {
      node.tags = Tags.push(node.tags, referenceTag);
    }
    node.tags = Tags.push(node.tags, buildInitializerTag(isArrayInitializer));
  ...
  if (!foundBinding && bindingTag) {
    node.tags = Tags.push(node.tags, bindingTag);
  }
  node.tags = Tags.push(node.tags, buildChild(Property, property));
...
export const shiftProperty = (node, property) => {
  ...
  if (!foundShift) {
    node.tags = Tags.push(node.tags, newShiftTag);
  }
  if (!foundBinding) {
    node.tags = Tags.push(node.tags, bindingTag);
  }
