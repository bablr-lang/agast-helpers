...
export const parseTagType = (input) => {
  ...
  switch (str[idx]) {
    ...
    case ':':
      return BindingTag;
    case '#':
      return str[idx + 1] === '#' ? IdTag : str[idx + 1] === '[' ? SumsTag : ReferenceTag;
    case '@':
      return EscapeTag;
    case '.':
    case '_':
...
export const parseNodeFlags = (input) => {
  ...
  let flags = cached || freezeRecord({ token });
