...
export const buildBoundsFromTags = (tags) => {
  ...
  if (firstProperty && firstProperty.value.node.type === TreeNode) {
    ...
    openStack = BTree.push(
      openStack,
      buildProperty([buildReferenceTag(), [], buildNode(buildGapTag())]),
    );
...
export const getRootProperty = (node) => {
  ...
  let tag = Tags.getAt(idx, node.value.tags);
...
export const isCover = (node) => {
  return node?.type === TreeNode && node.value.open.flags.cover;
};
...
export const isMultiFragment = (node) => {
  ...
  let { flags } = node.value.open;

  return flags.fragment && !flags.cover;
};
...
export const getOriginalFirstNode = (node) => {
  ...
  for (let child of Tags.traverse(node.value.tags)) {
...
export const getProperty = (pathSegment, node) => {
  ...
  return Tags.getAt(propIndex, node.value.tags);
};
...
export const getFlags = (node) => {
  return node.type === TreeNode ? node.value.open.flags : null;
};

export const getAttributes = (node) => {
  return node.type === TreeNode ? node.value.open.attributes : freeze({});
};
...
export const Path = class AgastPath extends WeakStackFrame {
  ...
  constructor(parent, node, parentIndex = null) {
    ...
    this.type = node.type === TreeNode ? node.value.open.type : null;
    this.flags = node.type === TreeNode ? node.value.open.flags : null;
    this.attributes = node.type === TreeNode ? node.value.open.attributes : null;

    let parentTag = parentIndex == null ? null : Tags.getAt(parentIndex, parent.node.value.tags);
  ...
  get coverBoundary() {
    ...
    if (path.parent && !getFlags(this.node)?.fragment && getFlags(path.parent.node).cover) {
      path = path.parent;
    }

    while (path && getFlags(path.node)?.cover && path.parent && getFlags(path.parent.node).cover) {
      path = path.parent;
    }
  ...
  get(path) {
    ...
    if (!(path.length && isObject(path[1]) && path[0].type === '_')) {
      while (!pathInst.node.value.open.type && pathInst.node.value.open.flags.cover) {
        if (pathInst.depth) throw new Error();
        skippedRootFragment = true;
        pathInst = pathInst.push(
          getRoot(pathInst.node, 1),
    ...
    for (let i = 0; i < path_.length; i++) {
      ...
      let { node } = Tags.getAt(tagsIndex, pathInst.node.value.tags).value;
  ...
  replaceWith(node, bindings) {
    ...
    while (path) {
      ...
      if (!held && !shift) {
        ...
        if (nextShift) {
          ...
          path = path.push(nextProperty.value.node, i);
    ...
    let resultPath = Path.from(built);

    for (let i = replacementPath.length - 1; i >= 0; i--) {
      let parentIndex = replacementPath[i];
      let property = Tags.getAt(parentIndex, resultPath.node.value.tags);
      resultPath = resultPath.push(property.value.node, parentIndex);
    }
  ...
  get held() {
    ...
    if (tagPath.tag.type !== Property || !tagPath.equalTo(this.firstPropertyTagPath)) return null;
    ...
    for (;;) {
      ...
      if (!coverParent?.node.value.open.flags.cover) break;
    }
  ...
  advance(tag) {
    ...
    if (tag.type === Property) {
      resultPath = Path.from(buildNode(Tags.push(resultPath.node.tags, tag)));
    } else {
  ...
  advanceUnwrapped(tag) {
    ...
    switch (tag.type) {
      case ReferenceTag: {
        let { type, isArray, name } = tag.value;
        ...
        if (targetPath.node.value.open.flags.cover && name != null) throw new Error();
        ...
        if (tag.value.name) {
          ...
          if (
            ...
            !referencesAreEqual(
              tag.value,
              Tags.getAt(rootIdx, targetPath.node.tags).value.property.reference,
            )
          ) {
            throw new Error('mismatched references');
          }
        ...
        targetPath = targetPath.replaceWith(
          buildNode(Tags.push(targetPath.node.value.tags, property)),
        );
        break;
      }
      ...
      case BindingTag: {
        ...
        let tagPropertyWrapper = buildChild(Property, buildProperty(tags, shift));

        targetPath = this.replaceWith(
          buildNode(Tags.replaceAt(propPath.tagsIndex, getTags(this.node), tagPropertyWrapper)),
        );
        break;
      }

      case OpenNodeTag: {
        let { literalValue, flags } = tag.value;
      ...
      case GapTag:
      case NullTag: {
        if (tag.type === GapTag && this.node.value.open.flags.token) {
          targetPath = this.replaceWith(buildNode(Tags.push(this.node.tags, tag)));
        } else {
      ...
      case CloseNodeTag: {
        ...
        if (lastChild?.type === Property && !wrapperIsFull(lastChild)) throw new Error();
        ...
        targetPath = targetPath.replaceWith(buildNode(Tags.push(node.value.tags, tag)));
        break;
      }
      ...
      case GapNode:
      case TreeNode: {
        let propertyNode = tag;
        let parentPath = targetPath;
        let { node: parentNode } = parentPath;

        let tagPath = TagPath.from(parentPath, -1);

        if (parentNode.value.open.flags.token) {
          if (tag.type === GapNode) throw new Error('use gap tags in tokens');
        }

        let propertyFlags = getFlags(propertyNode);
        if (propertyFlags?.fragment && !propertyFlags.cover) {
          ...
          let newTags = Tags.push(parentNode.value.tags, propertyNode.value.children);

          targetPath = this.replaceWith(buildNode(newTags));
          break;
        }
        ...
        let tags = [firstTag, bindingTags, held ? held.node : propertyNode];
        let tagPropertyWrapper = buildChild(Property, buildProperty(tags, shift));

        targetPath = this.replaceWith(
          buildNode(Tags.replaceAt(tagPath.tagsIndex, parentNode.value.tags, tagPropertyWrapper)),
        );
      ...
      case AttributeDefinition: {
        ...
        let openTag = getOpenTag(node);
        let { attributes } = node.value.open;
        ...
        targetPath = this.replaceWith(
          buildNode(Tags.push(Tags.replaceAt(0, node.value.tags, newOpenTag), tag)),
        );
        break;
      }

      case ShiftTag: {
        let { reference, shift: heldShift } = this.tagPathAt(-1).tag.value;

        if (!reference) {
          ({ reference } = this.tagPathAt(-1 - heldShift.index).tag.value);
        }
        ...
        let tagPropertyWrapper = buildChild(Property, buildProperty([tag], shift));

        targetPath = this.replaceWith(
          buildNode(Tags.push(this.node.value.tags, tagPropertyWrapper)),
        );
        break;
      }
...
export class TagPath {
  static from(path, tagsIndex, wrapperIndex) {
    ...
    let wrapperTag = Tags.getAt(index, tags);
  ...
  get previousSibling() {
    ...
    if (propertyIndex) {
      if (propertyIndex > 0) {
        ...
        let bindingIndex = wrapperIndex[1]?.index ?? property.value.bindings.length;
  ...
  get next() {
    ...
    for (;;) {
      ...
      if (tag.type === Property) {
        if (wrapperIndex.length === 0 && !leaving) {
      ...
      // done
      if (!isInitialTag && !isNodeTag(wrapperTag)) {
      ...
      if (nextSibling) {
        ...
        if (isNodeTag(nextSibling.tag)) {
          ...
          if (
            ...
            let shiftStack = tag.value.node.value.bounds[0];
          ...
          path = new Path(path, tag.value.node, tagsIndex);
          tagsIndex = 0;
          wrapperIndex = [];
          continue;
        } else {
  ...
  get nextUnshifted() {
    ...
    for (;;) {
      ...
      if (isArray(wrapperTag)) {
        ...
        continue;
      } else if (isNodeTag(wrapperTag) && !leaving) {
        path = path.push(wrapperTag, tagsIndex);
        tagsIndex = 0;
        wrapperIndex = [];
        continue;
      }
      ...
      if (!wasLeaving && wrapperTag.type === ReferenceTag && wrapperTag.value.flags.expression) {
        ...
        while (shiftedTag?.type === Property && shiftedTag.value.shift) {
          ...
          shiftedTag = Tags.getAt(tagsIndex + 1 + shifts, path.node.value.tags);
        }
      ...
      // done
      if (!isInitialTag && !isNodeTag(wrapperTag) && !isArray(wrapperTag)) {
        return TagPath.from(path, tagsIndex, wrapperIndex);
      }

      let { nextSibling } = TagPath.from(path, tagsIndex, wrapperIndex);
  ...
  get inner() {
    ...
    if (tag.type !== Property || !tag.value.node) {
      return null;
    }

    return this.path.push(tag.value.node, tagsIndex);
  }
