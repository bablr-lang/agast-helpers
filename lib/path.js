...
import { parseOpenNodeTag, parseTag, parseTagType, parseNodeFlags } from './parsers.js';
import {
  printNodeFlags,
...
export const buildProperty = (tags, shift) => {
  ...
  let { 0: ref, 1: bindings, 2: hash, 3: sums, 4: node } = tags[1];

  if (hash && !isString(hash)) throw new Error();
  if (sums && !isString(sums)) throw new Error();
  if (!BList.getType(tags) === Symbol.for('List')) throw new Error();
  if (!(ref === '' || (isString(ref) && [ReferenceTag, ShiftTag].includes(parseTagType(ref)))))
    throw new Error();
  if (tags[1].length > 1 && bindings !== '' && !isArray(bindings)) throw new Error();

  if (node && node.type === TreeNode && !Tags.startsNode(node.value.children[1][0]) && !sums)
    throw new Error();
  ...
  return freezeRecord({
    ...
    hash: hash ? parseTag(hash).value : null,
    sums: sums ? parseTag(sums).value : null,
    node: node || null,
...
export const buildNode = (tags) => {
  let tags_ = tags;
  let sigilTag;
  if (tags === '' || (isArray(tags) && (tags[0] === '' || tags[2] === ''))) throw new Error();
  if (isArray(tags_)) {
    if (tags[1][0] === null || tags[1][1] === null || parseTagType(tags[1][1]) === OpenNodeTag)
      throw new Error();
    if (!Number.isFinite(tags[0])) throw new Error();
    if (isArray(tags_[1][0]) && tags_[1][0][0] === 0) throw new Error();
    sigilTag = Tags.startsNode(tags_[1][0]) ? tags_[1][0] : '<__>';
  } else {
    if (!isString(tags)) throw new Error();
    let tag_ = parseTag(tags);
    let tag = printTag(tag_, porcelain);

    tags_ = Tags.fromValues([tag], flagsForSigilTag(tag_));
    sigilTag = tag;
  }
  ...
  switch (parseTagType(sigilTag)) {
    case GapTag:
      return buildTag(GapNode, freezeRecord({ tags: Tags.fromValues([sigilTag], '', 1) }));
    case NullTag:
      return buildTag(NullNode, freezeRecord({ tags: Tags.fromValues([sigilTag], '', 1) }));
    case OpenNodeTag: {
      let { flags, type, name, attributes, selfClosing } = parseOpenNodeTag(sigilTag).value;
      let children =
        tags_[1][1] || Tags.create((flags.object ? '{' : '') + (flags.array ? '[' : ''));

      if (
        flags.object !== Tags.getFlags(children).includes('{') ||
        flags.array !== Tags.getFlags(children).includes('[')
      ) {
        throw new Error();
      }

      if (
        flags.object !== Tags.getFlags(tags_).includes('{') ||
        flags.array !== Tags.getFlags(tags_).includes('[')
      ) {
        throw new Error();
      }
      ...
      if (Tags.getSize(tags_) > 1 && selfClosing) throw new Error();
...
let gapFrame = buildPathFrame(
  buildPropertyTag(BList.fromValues(['', '', '', '', buildNode('<//>')], 1)),
  null,
  true,
);
...
export const offsetForTag = (tag) => {
  switch (isString(tag) ? parseTagType(tag) : tag.type) {
    case ReferenceTag:
    case ShiftTag:
      return [0];
    case BindingTag:
...
export const buildSumsForNode = (node) => {
  ...
  return node.type === TreeNode ? printSums(Tags.buildSums(node.value.tags)) : '';
};
...
export const Path = class AgastPath {
  static from(node) {
    return (
      ...
      new Path(
        null,
        buildPathFrame(
          buildPropertyTag(BList.fromValues(['', '', '', buildSumsForNode(node), node], 1)),
        ),
      )
    );
  }
  ...
  static fromTag(openTag, hashTag) {
    return Path.from(
      buildNode(hashTag ? Tags.fromValues([hashTag, openTag], flagsForSigilTag(openTag)) : openTag),
    );
  }
  ...
  childPathAt(childrenIndex, propertyIndex) {
    let { children } = this;
    let absChildrenIdx = childrenIndex < 0 ? Tags.getSize(children) + childrenIndex : childrenIndex;
    return TagPath.from(this, absChildrenIdx + 1, propertyIndex);
  }

  childAt(childrenIndex, propertyIndex) {
    return this.childPathAt(childrenIndex, propertyIndex)?.tag;
  }
  ...
  replaceWith(node, bindingTags) {
    ...
    while (path) {
      ...
      if (shift) {
        newFirstTag = printTag(buildChild(ReferenceTag, targetReference), porcelain);
      }
      ...
      let tags_ = BList.fromValues(
        [
          ...
          targetBindings?.length
            ? BList.fromValues(
                map(
                  (binding) => printTag(buildChild(BindingTag, binding), porcelain),
      ...
      if (!held && !shift) {
        built = buildNode(
          Tags.replaceAt(targetParentIndex, buildPropertyTag(tags_), built.value.tags),
        );
        ...
        if (nextShift) {
          ...
          tags_ = BList.fromValues(
            [newFirstTag, tags_[1][1], tags_[1][2], tags_[1][3], tags_[1][4]],
            1,
          );
          ...
          built = buildNode(
            Tags.replaceAt(targetParentIndex, buildPropertyTag(tags_, shift), getTags(built)),
          );
          replacementPath.push(targetParentIndex);
        }
      }
  ...
  get held() {
    ...
    if (parseTagType(tags[1][0]) === ShiftTag && !tags[1][4]) {
  ...
  advance(tag) {
    ...
    if (tag_.type === LiteralTag) {
      ...
      if (
        ...
        lastPropPath.tag.value.tags[1].length < 4
      ) {
        if (
          (!lastPropPath || lastPropPath.tag.value.tags[1].length < 1) &&
          [
            BindingTag,
            HashTag,
            SumsTag,
        ...
        if (
          (!lastPropPath || lastPropPath.tag.value.tags[1].length < 2) &&
        ...
        if (
          (!lastPropPath || lastPropPath.tag.value.tags[1].length < 3) &&
        ...
        if ([OpenNodeTag, GapTag, NullTag, TreeNode, NullNode, GapNode].includes(tag_.type)) {
          resultPath = resultPath.advanceSingle(
            tag_.type === OpenNodeTag
              ? printSums(freezeRecord([0, 0, 0, tag_.type === GapNode ? 1 : 0]))
              : buildSumsForNode(tag_),
          ).path;
        }
      }

      if ([TreeNode, NullNode, GapNode].includes(tag_.type)) {
        resultPath = resultPath.advanceSingle(tag_).path;
      } else {
  ...
  advanceSingle(tag) {
    ...
    switch (tag_.type) {
      case ReferenceTag: {
        ...
        let property = buildPropertyTag(BList.fromValues([printTag(tag_, porcelain)]));
      ...
      case BindingTag: {
        ...
        let { 0: firstTag, 1: bindingTags = BList.create() } = propPath.value.tags[1];
        let tags = BList.fromValues([firstTag, BList.push(tag, bindingTags)], 1);
        let property = buildPropertyTag(tags, shift);
      ...
      case GapTag: {
        ...
        let node = buildNode(printTag(tag_));
      ...
      case TreeNode: {
        ...
        if (tagPath.type !== Property || tagPath.value.tags[1].length < 4) throw new Error();

        if (node.type === TreeNode) {
          propTagPath.type === SumsTag && propTagPath.tag !== Tags.buildSums(node.value.tags);
          if (
            (node.value.flags.array || node.value.flags.object) &&
            !(this.flags.array || this.flags.object)
          ) {
            throw new Error();
          }
        }
        ...
        } = tagPath.value.tags[1];
        let tags = BList.fromValues([firstTag, bindingTags, hashTag, sumsTag, node], 1);
        let property = buildPropertyTag(tags, shift);
      ...
      case AttributeDefinition: {
        ...
        targetPath = this.replaceWith(
          buildNode(
            Tags.push(tag, Tags.replaceAt(0, printTag(newOpenTag, porcelain), getTags(node))),
          ),
        );
        break;
      }
      ...
      case ShiftTag: {
        ...
        let property = buildPropertyTag(BList.fromValues([tag]), shift);
      ...
      case HashTag: {
        ...
        if (lastPropertyTag.value.tags[1].length < 2) throw new Error();
        ...
        let { 0: firstTag = '', 1: bindingTags = '' } = propPath ? propPath.value.tags[1] : [];
        let tags = BList.fromValues([firstTag, bindingTags, tag], 1);
        let property = buildPropertyTag(tags, shift);
      ...
      case SumsTag: {
        ...
        if (propPath.type !== Property || propPath.value.tags[1].length < 3) throw new Error();
        ...
        let {
          ...
          1: bindingTags = '',
          2: hashTag = '',
        } = propPath ? propPath.value.tags[1] : [];

        let tags = BList.fromValues([firstTag, bindingTags, hashTag, tag], 1);
        let property = buildPropertyTag(tags, shift);
      ...
      case EmptyTag: {
        ...
        let newProp = ![ReferenceTag, ShiftTag, BindingTag, EmptyTag, HashTag].includes(
          parseTagType(this.tagPathAt(-1, -1)),
        );
        let propPath = newProp ? null : this.tagPathAt(-1);

        let shift = propPath?.value.shift;

        let propTags = propPath ? propPath.value.tags[1] : [];
        let tags = BList.fromValues([...arrayValues(propTags), ''], 1);
        let property = buildPropertyTag(tags, shift);

        if (newProp) {
          targetPath = this.replaceWith(buildNode(Tags.push(property, getTags(this.node))));
        } else {
          targetPath = this.replaceWith(
            buildNode(Tags.replaceAt(propPath.tagsIndex, property, getTags(this.node))),
          );
...
export const findPropertyPath = (propertyIndex, property) => {
  let existing = freezeRecord({ existing: true });
  if (isArray(propertyIndex)) {
    return Tags.findPath(propertyIndex, property.value.tags, existing);
  }

  let propertyIndex_ = propertyIndex;

  let { tags } = property.value;
  let refSize = tags[1][0] ? 1 : 0;
  let bindingsSize = tags[1][1] ? Tags.getSize(tags[1][1]) : 0;
  let hashSize = tags[1][2] ? 1 : 0;
  let statsSize = tags[1][3] ? 1 : 0;
  let nodeSize = tags[1][4] ? 1 : 0;
  let valuesSize = BList.getValues(tags).length;
  let tagsSize = BList.getSize(tags);

  let refStop = refSize;
  let bindingsStop = refStop + bindingsSize;
  let hashStop = bindingsStop + hashSize;
  let statsStop = hashStop + statsSize;
  let nodeStop = refSize + bindingsSize + hashSize + statsSize + nodeSize;
  let absoluteIndex = propertyIndex_ < 0 ? propertyIndex_ + nodeStop : propertyIndex_;

  let idx = absoluteIndex;

  if (inRange(absoluteIndex, 0, refStop - 1)) {
    idx = [0];
  } else if (inRange(absoluteIndex, refStop, bindingsStop - 1)) {
    if (tags[1][1] === '') return null;
    let result = BList.findPath(absoluteIndex - 1, tags[1][1]).map(({ index }) => index);
    if (Number.isFinite(arrayLast(result))) {
      idx = [1].concat(result);
    }
  } else if (inRange(absoluteIndex, bindingsStop, hashStop - 1)) {
    idx = [2];
  } else if (inRange(absoluteIndex, hashStop, statsStop - 1)) {
    idx = [3];
  } else if (absoluteIndex <= nodeStop) {
    idx = [4];
  }

  if (idx?.length) {
    let tag = Tags.getAt(idx, property.value.tags);
    if (isArray(tag)) {
      idx = [];
    }
  }

  // let pathExists = isArray(propertyIndex_) && propertyIndex_?.length ? idx != null : propTag;
  return Tags.findPath(idx, property.value.tags, existing);
};

export class TagPath {
  static fromNode(node, tagsIndex = null, propertyIndex = null) {
    let sigilTag = Tags.getValues(getTags(node))[0];
  ...
  static from(path, tagsIndex, propertyIndex = null) {
    if (!path) throw new Error();
    let tagsPath = Tags.findPath(tagsIndex, getTags(path.node), freezeRecord({ existing: true }));

    if (!tagsPath) return null;

    let topPathFrame = tagsPath[tagsPath.length - 1];
    let tag = topPathFrame.node[1][topPathFrame.index];

    if (propertyIndex != null) {
      // we expect overspecifying the index to result in access, I think
      if (tag != null && (!isObject(tag) || tag.type !== Property)) {
        return new TagPath(path, tagsPath);
      }

      let propPath = findPropertyPath(propertyIndex, tag);

      if (!propPath) return null;

      return new TagPath(path, tagsPath, propPath);
    }

    return new TagPath(path, tagsPath);
  }

  constructor(path, tagsPath, propertyTagsPath = null) {
    ...
    let propTag = Tags.getAt(tagsPath, getTags(path.node));

    if (propertyTagsPath && propTag.type !== Property) throw new Error();

    let rawTag = propertyTagsPath ? Tags.getAt(propertyTagsPath, propTag.value.tags) : propTag;

    let tag = isString(rawTag) ? parseTag(rawTag) : rawTag;

    if (tag == null || isArray(tag)) throw new Error();

    let tagsIndex = Tags.indexForPath(tagsPath, getTags(path.node));

    if (tagsIndex == null) throw new Error();

    let propertyIndex = propertyTagsPath && Tags.indexForPath(propertyTagsPath, propTag.value.tags);
  ...
  get nextSibling() {
    let { path, tagsIndex, propertyTagsPath, propertyIndex } = this;
    ...
    switch (propertyIndex0) {
      ...
      case 3: {
        ...
        let tagPath = TagPath.from(path, tagsIndex, this.propertyIndex + i);
        while (tagPath && tagPath.tag === '') {
          i++;
          tagPath = TagPath.from(path, tagsIndex, this.propertyIndex + i);
        }
        if (tagPath) {
          return tagPath;
        }
      ...
      case 4:
      default: {
        if (!nextChild || nextChild.type !== Property || !propertyTagsPath) {
  ...
  get previousSibling() {
    ...
    if (propertyIndex0) {
      if (propertyIndex0 > 0) {
        ...
        let bindingIndex = propertyTagsPath[1]?.index ?? property.value.bindings.length;
  ...
  get next() {
    ...
    for (;;) {
      let rawTag = Tags.getAt(tagsIndex, getTags(path.node));
      let tag = isObject(rawTag) ? rawTag : parseTag(rawTag);
      let propTag = tag;
      let lastRef = null;
      ...
      if (isObject(tag) && tag.type === Property && !wasLeaving) {
        if (propertyIndex.length === 0) {
          let { 0: ref, 1: bindings, 2: hash, 3: sums } = tag.value.tags[1];
        ...
        if (propertyIndex.length === 1) {
          ...
          while ((!propTag || propTag.type === EmptyTag) && node[1][index + 1]) {
            propertyIndex[0] = { index: index + 1, node };
            propTag = node[1][index + 1];
            if (index === tag.value.tags[1].length - 1) break;
      ...
      if (!isInitialTag && !isNodeTag(propTag) && propTag.type !== Property) {
        return TagPath.from(path, tagsIndex, propertyIndex.length ? propertyIndex : null);
      }
      ...
      if (nextSibling) {
        ({ tagsIndex, propertyTagsPath } = nextSibling);
