...
export const isFragmentNode = (node) => {
  return node && node.type === null && node.flags.fragment;
};
  ...
  advance(tag) {
    ...
    switch (tag.type) {
      ...
      case OpenNodeTag: {
        ...
        if (!this.node.flags.token || (tag.type === Property && tag.value.reference.type === '@')) {
      ...
      case Node: {
        ...
        if (propertyNode.flags.fragment && !propertyNode.flags.cover) {
          ...
          if (parentAtLeftSide) {
            openStack = BTree.pop(openStack);
            // throw new Error('not implemented');
            // openStack = BTree.push(openStack, propertyNode.value);
        ...
        if (parentAtLeftSide && nodeIsComplete(propertyNode)) {
          openStack = BTree.pop(openStack);
    ...
    for (;;) {
      ...
      if (tag.type === PropertyWrapper) {
        if (wrapperIndex + 1 < tag.value.tags.length) {
          wrapperIndex++;
          continue;
        } else if (!wasLeaving && tag.value.tags[1]) {
