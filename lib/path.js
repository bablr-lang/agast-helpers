...
import { buildBindingTag, buildChild } from './builders.js';
...
export const Path = class AgastPath extends WeakStackFrame {
  ...
  replaceWith(node, binding = buildBindingTag().value) {
    let { childrenIdx } = this;
    let built = node;
    ...
    while (true) {
      built = { ...parent.node };
      built.children = Children.replaceAt(childrenIdx + 1, built.children, buildChild(binding));
