...
let gapFrame = buildPathFrame(
  buildPropertyTag(
    Tags.fromValues([Tags.fromValues([]), Tags.fromValues([]), buildNode('<//>')], 1),
  ),
  null,
  true,
);
...
export const Path = class AgastPath {
  static from(node) {
    return (
      ...
      new Path(
        null,
        buildPathFrame(
          buildPropertyTag(Tags.fromValues([Tags.fromValues([]), Tags.fromValues([]), node], 1)),
        ),
      )
    );
  }
  ...
  get reference() {
    return parseTag(this.referenceTag)?.value;
  }
    ...
    while (path) {
      ...
      if (!held && !shift) {
        ...
        if (nextShift) {
          ...
          tags_ = Tags.fromValues([newFirstTag, tags_[1][1], tags_[1][2]], 1);
  ...
  advanceUnwrapped(tag) {
    ...
    switch (tag_.type) {
      case ReferenceTag: {
        ...
        let property = buildChild(Property, buildProperty(Tags.fromValues([printTag(tag)])));
      ...
      case BindingTag: {
        ...
        if (!tag_.value.segments) throw new Error();
        ...
        let tags = Tags.fromValues([firstTag, Tags.from(...bindingTags, tag_)], 1);
      ...
      case OpenNodeTag: {
        let { literalValue, flags, type } = tag_.value;
      ...
      case TreeNode: {
        ...
        if (propertyIsFull(tagPath.tag)) {
          let tags = Tags.fromValues([Tags.fromValues([]), Tags.fromValues([]), node], 1);
          let property = buildChild(Property, buildProperty(tags, shift));
          ...
          let tags = Tags.fromValues([firstTag, bindingTags, node], 1);
          let property = buildChild(Property, buildProperty(tags, shift));
...
export class TagPath {
  ...
  //   let propTag = parseTag(Tags.getAt(index, tags));
  ...
  //   if (isObject(propTag) && propertyIndex_ != null) {
  //     if (isArray(propertyIndex) && propertyIndex.length > 1) {
  //       resolvedProperty = Tags.findPath(propertyIndex_.slice(0, 1), propTag.value.tags);
  //       let lastSegment = resolvedProperty && arrayLast(resolvedProperty);
  //       let resolvedValue = lastSegment?.node[1][lastSegment.index];
  //       if (Tags.isNode(resolvedValue)) {
  //         let newResolved = Tags.findPath(propertyIndex_.slice(1), resolvedValue);

  //         resolvedProperty = newResolved && resolvedProperty.concat(newResolved);
  //       }
  //     } else {
  //       resolvedProperty = Tags.findPath(propertyIndex_, propTag.value.tags);
  //     }
  //   }

  //   let resolvedPropertyIndex = deepFreeze((resolvedProperty || []).map(({ index }) => index));

  //   if (resolvedPropertyIndex?.length && Number.isFinite(arrayLast(resolvedPropertyIndex))) {
  //     pathExists = true;
  //   }
  ...
  constructor(path, tagsIndex, propertyIndex) {
    if (!isImmutable(propertyIndex)) throw new Error();
    ...
    for (let index of propertyIndex) {
    ...
    for (;;) {
      ...
      if (isObject(tag) && tag.type === Property && !wasLeaving) {
        ...
        if (propertyIndex != null) {
          propTag = parseTag(Tags.getAt(propertyIndex, tag.value.tags));
          let lastRef_ = Tags.getAt([0], tag.value.tags);
          lastRef = isArray(lastRef_) ? buildReferenceTag() : parseTag(lastRef_);
        }
      }
      ...
      let { nextSibling } = TagPath.from(path, tagsIndex, freeze([...propertyIndex]));
      ...
      if (nextSibling) {
        ...
        if (isNodeTag(nextSibling.tag)) {
          ...
          propertyIndex = [];
          continue;
        } else if (nextSibling.type === Property) {
          continue;
        } else {
          return nextSibling;
        }
