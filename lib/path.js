...
export const buildNode = (tags) => {
  ...
  if (isArray(tags_)) {
    ...
    let tag = printTag(tags, porcelain);
...
export const countList = (name, node) => {
  ...
  return Tags.findStats(printTag(buildReferenceTag(null, name)), Tags.getSums(getTags(node)).refs);
};
    ...
    switch (tag_.type) {
      case ReferenceTag: {
        ...
        let property = buildChild(
          Property,
          buildProperty(BList.fromValues([printTag(tag, porcelain)])),
        );
