...
export const getPropertyTagsIndex = (agAstNode, type, name, index, shiftIndex) => {
  ...
  if (ref?.isArray) {
    if (index < 0) {
      index = Tags.getAtName(type, counts) - initializerOffset + index;
    } else if (index == null) {
      index = Tags.getAtName(name, counts) - 1 - initializerOffset;
...
export const getFirstNodeProperty = (node) => {
  for (let child of Tags.traverse(node.tags)) {
    if (child.type === PropertyWrapper && child.value.tags[1]?.type !== InitializerTag) {
...
export const Path = class AgastPath extends WeakStackFrame {
  ...
  get referenceTagPath() {
    ...
    if (path.tag.type === ShiftTag) {
      let propertyPath = new TagPath(this.parent, this.parentIndex);
      let { property } = propertyPath.child.value;
      path = new TagPath(this.parent, this.parentIndex - property.shift.index, 0);
    }
    return path;
  }
  ...
  get held() {
    ...
    if (tagPath.tag.type !== PropertyWrapper || !wrapperIsFull(tagPath.tag)) return false;

    let refPath = this.tagPathAt(-1, 0);

    return refPath.tag.type === ShiftTag;
  }
  ...
  advanceUnwrapped(tag) {
    ...
    switch (tag.type) {
      case ReferenceTag: {
        ...
        if (!this.node.flags.hasGap && flags.hasGap) {
            ...
            if (count === 1) {
              ...
              if (!property && has(key, this.node)) {
                ...
                if (!ref.value.isArray) {
                  this.advance(buildNodeTag(buildNullNode()));
                }
              }
            }
          }
      ...
      case Node: {
        ...
        if (propertyNode.flags.fragment && !propertyNode.flags.cover) {
          this.node.tags = Tags.push(this.node.tags, propertyNode.children);
          this.node.children = this.node.tags[1][1];
        ...
        if (node.flags.token) {
          if (isGapNode(propertyNode)) throw new Error();
        }

        let propPath = TagPath.from(this, -1);
        let refPath = TagPath.from(this, -1, 0);
        let refOrShift = refPath.tag;
        let { property } = propPath.tag.value;

        if (!node.flags.token && ![ShiftTag, ReferenceTag].includes(refOrShift.type))
          throw new Error();
        ...
        if (this.held) {
          if (isGapNode(propertyNode)) {
            ...
            let matchesHeld =
              ...
              this.held.node === propertyNode ||
              this.held.node === getOriginalFirstNode(propertyNode, (height ?? 1) - 1);
            if (!matchesHeld) {
              throw new Error();
            }
          }
        ...
        this.node.tags = Tags.replaceAt(refPath.tagsIndex, this.node.tags, tagPropertyWrapper);
        this.node.children = this.node.tags[1][1];
      ...
      case InitializerTag: {
        ...
        let refTag = this.tagPathAt(-1, 0).tag;

        if (refTag.type !== ReferenceTag && this.parent) {
        ...
        if (getInitializerTagsIndex(node, refTag) != null) {
          throw new Error();
        }
