...
export const Path = class AgastPath {
  ...
  advanceUnwrapped(tag) {
    ...
    switch (tag_.type) {
      ...
      case HashTag:
        let lastPropertyPath = this.tagPathAt(-1);

        if (lastPropertyPath.type !== Property) throw new Error();

        let { shift } = lastPropertyPath.value;
        let { 0: firstTag, 1: bindingTags, 2: node } = lastPropertyPath.value.tags[1];

        if (!node) throw new Error();

        let property = buildChild(
          Property,
          buildProperty(BList.fromValues([firstTag, bindingTags, node, tag], 1), shift),
        );

        targetPath = this.replaceWith(
          buildNode(Tags.replaceAt(lastPropertyPath.tagsIndex, property, getTags(this.node))),
        );
        break;
