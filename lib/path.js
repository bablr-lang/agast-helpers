...
export const buildNode = (tags) => {
  ...
  let tags_ = isArray(tags) ? tags : Tags.fromValues([Tags.empty(), printTag(tags)], 1);

  if (!Number.isFinite(tags_[0])) throw new Error();
  if (!isArray(tags_[1]) || !tags_[1].length) throw new Error();

  let hashTag = tags_[1][0] || Tags.empty();
  let sigilTag = tags_[1][1];
...
let gapFrame = buildPathFrame(
  buildPropertyTag(BList.fromValues([BList.empty(), BList.empty(), buildNode('<//>')], 1)),
  null,
  true,
);
...
export const getSigilTag = (node) => {
  return Tags.getValues(getTags(node))[1];
};

export const getOpenTag = (node) => {
  let tag = Tags.getValues(getTags(node))[1];
...
export const Path = class AgastPath {
  static from(node) {
    return (
      ...
      new Path(
        null,
        buildPathFrame(buildPropertyTag(BList.fromValues([BList.empty(), BList.empty(), node], 1))),
      )
    );
  }
  ...
  get children() {
    let { node } = this;
    return node.type === TreeNode ? getTags(this.node)[1][2] : null;
  }
    ...
    switch (tag_.type) {
      ...
      case TreeNode: {
        ...
        if (propertyIsFull(tagPath.tag)) {
          let tags = BList.fromValues([BList.empty(), BList.empty(), node], 1);
          let property = buildChild(Property, buildProperty(tags, shift));

          targetPath = this.replaceWith(buildNode(Tags.push(property, parentNode.value.tags)));
        } else {
          let { 0: firstTag, 1: bindingTags = BList.empty() } = tagPath.value.tags[1];
...
export class TagPath {
  ...
  static fromNode(node, tagsIndex = null, propertyIndex = null) {
    let firstTag = Tags.getAt(0, getTags(node));
    let tagsIndex_ = tagsIndex === null && parseTagType(firstTag) === HashTag ? 0 : 1;
  ...
  get nextSibling() {
    ...
    if (tagsIndex === 0) return TagPath.from(path, tagsIndex + 1);
    ...
    for (;;) {
      ...
      if (isObject(tag) && tag.type === Property && !wasLeaving) {
        if (propertyIndex.length === 0) {
          ...
          let index = !isArray(ref) ? 0 : !isArray(bindings) ? 1 : 2;
