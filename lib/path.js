...
export const buildNode = (tags) => {
  ...
  if (isArray(tags_)) {
    ...
    tags_ = Tags.fromValues([tag === '<__>' ? '' : tag], 1);
    sigilTag = tag;
  }
...
export const getSigilTag = (node) => {
  return node.value.type === Symbol.for('__') ? '<__>' : Tags.getValues(getTags(node))[0];
};

export const getOpenTag = (node) => {
  let tag = node.value.type === Symbol.for('__') ? '<__>' : Tags.getValues(getTags(node))[0];
...
export const countList = (name, node) => {
  ...
  return Tags.findSums(printTag(buildReferenceTag(null, name)), Tags.getSums(getTags(node)).refs);
};
...
export const Path = class AgastPath {
  ...
  advanceSingle(tag) {
    ...
    switch (tag_.type) {
      ...
      case GapTag: {
        if (![ReferenceTag, HashTag].includes(this.tagPathAt(-1, 0)?.type)) {
      ...
      case CloseNodeTag: {
        ...
        let isFrag = openTag.value.type === Symbol.for('__');

        targetPath = targetPath.replaceWith(buildNode(Tags.push(isFrag ? '' : tag, getTags(node))));
      ...
      case TreeNode: {
        ...
        if (propertyIsFull(tagPath.tag)) {
          ...
          let { 0: firstTag, 1: bindingTags = '' } = tagPath.value.tags[1];
      ...
      case HashTag:
        let newProp = ![ReferenceTag, ShiftTag].includes(parseTagType(this.tagPathAt(-1, -1)));
