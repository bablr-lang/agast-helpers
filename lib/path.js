...
export const Path = class AgastPath extends WeakStackFrame {
  ...
  get(path, shiftIndex = null) {
    ...
    for (let i = 0; i < path_.length; i++) {
      ...
      let shiftOffset =
        (shiftIndex != null && i === path_.length - 1
          ? shiftIndex < 0
            ? btree.getSize(node) + shiftIndex
            : shiftIndex
          : 0) * 2;
  ...
  get nextUnshifted() {
    ...
    for (;;) {
      ...
      if (tag.type === ReferenceTag && tag.value.flags.expression) {
        ...
        } while (tag_ && [GapTag, ShiftTag].includes(tag_.type));
        if (!tag_) {
          return null;
        }
      }
      ...
      if (tag.type === GapTag && !wasLeaving && !isGapNode(path.node)) {
        ...
        if (prevTag.type === ShiftTag) {
          ...
          if (
            ...
            if (refTag) {
              path = path.get(pathDesc, refTag.value.flags.expression ? -1 : null);
              childrenIndex = 0;
