...
export const isCover = (node) => {
  return node?.type === TreeNode && node.value.flags.cover;
};
...
export const Path = class AgastPath extends WeakStackFrame {
  ...
  get(path) {
    ...
    if (!(path.length && isObject(path[1]) && path[0].type === '_')) {
      while (!pathInst.node.value.type && pathInst.node.value.flags.cover) {
  ...
  get held() {
    ...
    for (;;) {
      ...
      if (!coverParent?.node.value.flags.cover) break;
    }
  ...
  advanceUnwrapped(tag) {
    ...
    switch (tag.type) {
      case ReferenceTag: {
        ...
        if (targetPath.node.value.flags.cover && name != null) throw new Error();
      ...
      case GapTag:
      case NullTag: {
        if (tag.type === GapTag && this.node.value.flags.token) {
      ...
      case TreeNode: {
        ...
        if (parentNode.value.flags.token) {
