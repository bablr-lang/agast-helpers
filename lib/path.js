...
import { printNodeFlags, printReference, printTag } from './print.js';
...
export const buildNode = (tags) => {
  ...
  switch (parseTagType(sigilTag)) {
    ...
    case OpenNodeTag: {
      ...
      let children = tags_[1][1] || Tags.empty();
    ...
    switch (tag_.type) {
      case ReferenceTag: {
        ...
        if (targetPath.flags.object) {
          ...
          if (propertyPath && printReference(propertyPath.tag.value.reference) >= printTag(tag_)) {
            throw new Error();
          }
      ...
      case BindingTag: {
        ...
        let { 0: firstTag, 1: bindingTags = BList.empty() } = propPath.value.tags[1];
