...
export const Path = class AgastPath {
  ...
  removeAt(path) {
    let built = null;
    let path_ = this.get(path).parent;
    let lastPath = path_;
    let replacementPath = [];
    ...
    let targetShift_ = path_.parent?.parentProperty?.value.tags[1][0];
    let targetShift = targetShift_ && (targetShift_.type === ReferenceTag ? null : targetShift_);
    let targetReference = path_.reference;
    let targetBindings = path_.bindings;
    let targetParentIndex = path_.parentIndex;
    let held = null;
    let heldCount = 0;
    let value;
