...
export const Path = class AgastPath {
  ...
  constructor(parent, frame) {
    ...
    let { property, parentIndex, isGap } = frame;
    ...
    if (!node) throw new Error();
    if (!Object.isFrozen(frame)) throw new Error();
    ...
    if (
      parent &&
      (isGap
        ? BTree.getAt(0, parent.node.value.bounds[0]).property
        : parent.tagPathAt(parentIndex).tag) !== property
    ) {
