...
let gapFrame = buildPathFrame(
  buildPropertyTag(BList.fromValues([Tags.empty(), Tags.empty(), buildNode('<//>')], 1)),
  null,
  true,
);
...
export const Path = class AgastPath {
  static from(node) {
    return (
      ...
      new Path(
        null,
        buildPathFrame(buildPropertyTag(BList.fromValues([Tags.empty(), Tags.empty(), node], 1))),
      )
    );
  }
    ...
    switch (tag_.type) {
      ...
      case TreeNode: {
        ...
        if (propertyIsFull(tagPath.tag)) {
          let tags = BList.fromValues([Tags.empty(), Tags.empty(), node], 1);
          let property = buildChild(Property, buildProperty(tags, shift));

          targetPath = this.replaceWith(buildNode(Tags.push(property, parentNode.value.tags)));
        } else {
          let { 0: firstTag, 1: bindingTags = Tags.empty() } = tagPath.value.tags[1];
