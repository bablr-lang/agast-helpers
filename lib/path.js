...
export const getOriginalFirstNode = (node) => {
  for (let child of sumtree.traverse(node.children)) {
    if (child.type === Property) {
      return getProperties(buildPathSegment(child.value.reference.name, -1, 1), node.properties)
        .node;
    }
  }
  return null;
...
export const getFirstNodeShiftStack = (node) => {
  for (let child of sumtree.traverse(node.children)) {
    if (child.type === Property) {
      ...
      if (!reference.flags.isArray) return null;
...
export class TagPath {
    ...
    for (;;) {
      ...
      if (tag.type === Property && !wasLeaving) {
        ...
        if (
          ...
          let gapNode = btree.getAt(0, path.node.properties[ref.name].node).node;

          if (ref.isArray) {
            gapNode = btree.getAt(-1, gapNode.node);
          }
      ...
      if (tag.type === BindingTag && prevTag.type === ShiftTag) {
        ...
        pathDesc.shiftIndex = prevTag.value.index + 1;
  ...
  get inner() {
    ...
    if (refPath.tag.type === ShiftTag) {
      shiftPath = refPath;
      refPath = TagPath.from(this.path, this.childrenIndex - refPath.tag.value.index * 3 - 1);
    }
