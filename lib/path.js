...
import { deepFreezeRecord, freezeRecord, recordValues } from '@bablr/record';
...
export const buildBoundsFromTags = (tags) => {
  ...
  if (parseTagType(Tags.getAt(-1, tags)) !== CloseTag) {
...
export const endsNode = (tag) => {
  ...
  if ([CloseTag, NullTag, GapTag].includes(tagType)) return true;
...
export const Path = class AgastPath {
  ...
  get closeTagPath() {
    ...
    return tagPath.type === CloseTag ? tagPath : null;
  }
  ...
  advance(tag) {
    ...
    if (
      (tag_.type === CloseTag || (tag_.type === OpenNodeTag && tag_.value.selfClosing)) &&
      resultPath.depth
    ) {
...
export class TagPath {
  static from(path, tagsIndex, propertyIndex) {
    ...
    if (
      [OpenNodeTag, CloseTag, LiteralTag, AttributeDefinition, GapTag, NullTag].includes(
        propTag?.type,
      )
    ) {
      propertyIndex_ = null;
  ...
  constructor(path, tagsIndex, propertyIndex) {
    if (!isDeepRecord(propertyIndex)) throw new Error();
  ...
  get nextSibling() {
    ...
    if (propertyIndex0 == null && !nextChild) {
      return this.type === CloseTag ? null : path.closeTagPath;
    }
