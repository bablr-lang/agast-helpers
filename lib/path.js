...
export const buildNode = (tags) => {
  ...
  if (isArray(tags_)) {
    ...
    tags_ = Tags.fromValues([tag === '<__>' ? null : tag], 1);
    sigilTag = tag;
  }
...
let gapFrame = buildPathFrame(
  buildPropertyTag(BList.fromValues([null, null, null, buildNode('<//>')], 1)),
  null,
  true,
);
...
export const Path = class AgastPath {
  static from(node) {
    return (
      ...
      new Path(
        null,
        buildPathFrame(buildPropertyTag(BList.fromValues([null, null, null, node], 1))),
      )
    );
  }
    ...
    switch (tag_.type) {
      ...
      case TreeNode: {
        ...
        if (propertyIsFull(tagPath.tag)) {
          let tags = BList.fromValues([null, null, null, node], 1);
          let property = buildChild(Property, buildProperty(tags, shift));
          ...
          let tags = BList.fromValues([firstTag, bindingTags, null, node], 1);
          let property = buildChild(Property, buildProperty(tags, shift));
...
export class TagPath {
  static from(path, tagsIndex, propertyIndex) {
    ...
    let childrenIndex =
      children && index >= 1 && index < 1 + Tags.getSize(children) ? index - 1 : null;
    let propTag_ =
