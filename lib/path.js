...
export const propertyIsFull = (wrapper) => {
...
export const isFragment = (node) => {
  return node && node.value.flags.fragment;
};
...
export const Path = class AgastPath extends WeakStackFrame {
  ...
  getBindings(childrenIndex) {
    ...
    let wrapper = this.getPropertyTagPath(childrenIndex);
  ...
  get(path) {
    ...
    if (!(path.length && isObject(path[1]) && path[0].type === '_')) {
      while (!pathInst.node.value.name && pathInst.node.value.flags.cover) {
    ...
    for (let i = 0; i < path_.length; i++) {
      ...
      let { node } = Tags.getAt(tagsIndex, getTags(pathInst.node)).value;
  ...
  mapOnto(rootNode) {
    ...
    for (let i = 1; i <= this.depth; i++) {
      ...
      let { node } = Tags.getAt(parentIndex, path.node.value.tags).value;
      path = path.push(node, parentIndex);
    }
  ...
  get held() {
    ...
    if (propertyIsFull(tagPath.tag) || (tagPath.inner && nodeIsComplete(tagPath.inner.node))) {
      return null;
    }
...
export class TagPath {
  static from(path, tagsIndex, propertyIndex) {
    ...
    if ([OpenNodeTag, CloseNodeTag, LiteralTag].includes(propTag?.type)) {
      propertyIndex_ = [];
    }
  ...
  get nextUnshifted() {
    ...
    for (;;) {
      ...
      if (isArray(propTag)) {
        ...
        path = path.push(propTag, tagsIndex);
        tagsIndex = 0;
        propertyIndex = [];
        continue;
      }
  ...
  get inner() {
    ...
    return node && this.path.push(node, tagsIndex);
  }
