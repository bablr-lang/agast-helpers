...
export const buildBoundsFromTags = (tags) => {
  let gapFrame = buildPathFrame(
    buildPropertyTag(Tags.fromValues([null, Tags.fromValues([]), buildNode('<//>')])),
    null,
    true,
  );
...
export const Path = class AgastPath {
  static from(node) {
    return (
      ...
      new Path(
        null,
        buildPathFrame(
          buildPropertyTag(Tags.fromValues([buildReferenceTag(), Tags.fromValues([]), node])),
        ),
      )
    );
  }
  ...
  get openTagPath() {
    ...
    return parseTagType(tagPath.tag) === OpenNodeTag ? tagPath : null;
  }

  get closeTagPath() {
    let tagPath = TagPath.from(this, -1);
    return parseTagType(tagPath.tag) === CloseNodeTag ? tagPath : null;
  }
  ...
  advance(tag) {
    ...
    switch (tag_.type) {
      ...
      case GapTag:
      case OpenNodeTag: {
        let sigilTag = parseTag(getSigilTag_(tag));
        ...
        let reference = lastTagPath?.referenceTagPath.value;

        // TODO revisit this condition
        if (!getFlags(this.node)?.token || reference?.type === '@' || tag_.type === GapTag) {
          if (newProperty || !lastTagPath.value.tags.length) {
            resultPath = this.advanceUnwrapped(buildReferenceTag()).path;
          }
        }
    ...
    if (
      (tag_.type === CloseNodeTag ||
        (tag_.type === OpenNodeTag && parseTag(tag).value.selfClosing)) &&
      resultPath.depth
    ) {
  ...
  advanceUnwrapped(tag) {
    ...
    switch (tag_.type) {
      case ReferenceTag: {
        ...
        let property = buildChild(Property, buildProperty(Tags.fromValues([tag_])));
      ...
      case BindingTag: {
        ...
        let tags = Tags.fromValues([firstTag, Tags.from(...bindingTags, tag_)]);
      ...
      case ShiftTag: {
        ...
        let property = buildChild(Property, buildProperty(Tags.fromValues([tag_]), shift));
...
export class TagPath {
  static from(path, tagsIndex, propertyIndex) {
    ...
    let resolvedPropertyIndex =
      isObject(propTag) && propertyIndex_ != null
        ? Tags.findPath(propertyIndex_, propTag.value.tags)
        : [];
  ...
  constructor(path, tagsIndex, propertyIndex) {
    ...
    if (isObject(tag) && tag.type === Property && propertyIndex_.length) throw new Error();
    ...
    for (;;) {
      ...
      if (isObject(tag) && tag.type === Property) {
        ...
        if (propertyIndex != null) {
          ...
          lastRef = parseTag(Tags.getAt(0, tag.value.tags));
        }
      }
      ...
      if (isArray(propTag)) {
        if (propTag.length) {
          ...
          propTag = parseTag(propTag[propertyIndex[1].index]);
        } else {
