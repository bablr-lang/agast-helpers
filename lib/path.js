...
export const Path = class AgastPath extends WeakStackFrame {
  ...
  get bindingTag() {
    return this.bindingTagPath?.tag;
  }
  ...
  replaceWith(node, binding = buildBinding()) {
    ...
    while (path) {
      ...
      let tags_ = [tags[0], buildChild(BindingTag, targetBinding)];
...
export class TagPath {
  ...
  static from(path, tagsIndex, offsetIndex) {
    ...
    let offset =
      offsetIndex == null || tag.type !== PropertyWrapper
        ? null
        : offsetIndex < 0
  ...
  get next() {
    ...
    for (;;) {
      ...
      offsetIndex = tag.type === PropertyWrapper ? (offsetIndex == null ? 0 : offsetIndex) : null;
      ...
      // in
      if (tag.type === PropertyWrapper && !wasLeaving) {
        if (offsetIndex + 1 < tag.value.tags.length) {
          offsetIndex++;
          continue;
        } else if (tag.value.tags[1].type === InitializerTag) {
        } else {
          // jump to gap tag?
          let tag0;
          if (
            ...
            (tagsIndex === 0 ||
              (tagsIndex === 1 &&
                (tag0 = Tags.getAt(0, path.node.tags)).type === PropertyWrapper &&
