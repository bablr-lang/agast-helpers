...
export const Path = class AgastPath extends WeakStackFrame {
  ...
  get held() {
    ...
    if (tagPath.tag.type !== PropertyWrapper) return null;
...
export class TagPath {
  ...
  get nextUnshifted() {
    ...
    for (;;) {
      ...
      if (tag.type === PropertyWrapper && wrapperIndex.length === 0) {
        wrapperIndex.push({ index: 0, node: tag.value.tags });
      }
      ...
      while (isArray(wrapperValue) && !wrapperValue.length) {
        wrapperIndex[0].index++;
        wrapperValue = Tags.getAt([wrapperIndex[0].index], tag.value.tags);
      }

      if (isArray(wrapperValue) && wrapperValue.length && wrapperIndex.length === 1) {
        // enter bindings array
