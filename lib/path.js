...
const { hasOwn, freeze, deepFreeze } = Object;
const { isArray } = Array;
...
export const buildBoundsFromTags = (tags) => {
  ...
  let leading = BTree.fromValues([gapFrame]);
  ...
  if (firstProperty && firstProperty.value.node.type === TreeNode) {
    ...
    let rootFrame = BTree.getAt(1, leading);

    leading = BTree.replaceAt(0, leading, buildPathFrame(firstProperty));
    if (rootFrame) {
      leading = BTree.replaceAt(1, leading, buildPathFrame(rootFrame.property, firstPropertyIndex));
    }
    leading = BTree.unshift(gapFrame, leading);
  }
  ...
  if (lastProperty && lastProperty.value.node.type === TreeNode) {
    ...
    let rootFrame = BTree.getAt(1, trailing);

    trailing = BTree.replaceAt(0, trailing, buildPathFrame(lastProperty));
    if (rootFrame) {
      trailing = BTree.replaceAt(
        1,
        trailing,
        buildPathFrame(rootFrame.property, lastPropertyIndex),
      );
    }
    trailing = BTree.unshift(gapFrame, trailing);
  }
...
export const Path = class AgastPath {
  ...
  constructor(parent, frame) {
    ...
    if (
      parent_ &&
      (isGap
        ? BTree.getAt(0, parent_.node.value.bounds.leading).property
  ...
  push(tagsIndex, isGap) {
    let tag = isGap
      ? BTree.getAt(0, this.node.value.bounds.leading).property
      : tagsIndex != null
  ...
  replaceWith(node, bindingTags) {
    ...
    while (path) {
      ...
      if (!held && !shift) {
        ...
        if (nextShift) {
          let { reference, bindings } = BTree.getAt(
            -1,
        ...
        if (held) {
          ...
          if (getRoot(node) && nodeIsComplete(node)) {
            matchesHeld =
              ...
              held.node ===
                BTree.getAt(
                  -BTree.getSize(held.node.value.bounds.leading),
