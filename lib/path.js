...
export const buildNode = (tags) => {
  ...
  if (isArray(tags_)) {
    ...
    let tag = printTag(isString(tags) ? parseTag(tags) : tags, porcelain);

    tags_ = Tags.fromValues([tag], 1);
    sigilTag = tag;
  }
  ...
  switch (parseTagType(sigilTag)) {
    ...
    case OpenNodeTag: {
      ...
      let children = tags_[1][1] || Tags.create();
      ...
      case TreeNode: {
        ...
        if (node.type === TreeNode) {
          propTagPath.type === SumsTag &&
            propTagPath.tag !== printSums(Tags.getSums(node.value.children));
          if (
