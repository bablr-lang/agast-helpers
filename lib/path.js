...
import { printNodeFlags, printReference, printSums, printTag } from './print.js';
...
export const flagsForSigilTag = (tag) => {
  return tag.type === TreeNode
    ? (tag.value.flags.object ? '{' : '') + (tag.value.flags.array ? '[' : '')
    : '';
};
...
export const isAnonymousToken = (node) => {
  return node.value.flags.token && !node.value.name;
};
...
export const countList = (name, node) => {
  ...
  return Tags.findCount(
    printTag(buildReferenceTag(null, name)),
    Tags.buildRefCounts(getTags(node)),
  );
};
...
export const Path = class AgastPath {
  static from(node) {
    return (
      node &&
      new Path(null, buildPathFrame(buildPropertyTag(BList.fromValues(['', '', '', '', node], 1))))
    );
  }
  ...
  advance(tag) {
    ...
    if (tag_.type === LiteralTag) {
      ...
      if (
        tag_.type === BindingTag &&
        (lastPropPath?.type !== Property || propertyIsFull(lastPropPath.tag))
      ) {
        ...
        (lastPropPath?.type !== Property ||
          ...
          lastPropPath.tag.value.tags[1].length < 2)
      ) {
        if (lastPropPath.tag.value.tags[1].length < 1) {
          resultPath = resultPath.advanceSingle('.:').path;
        }
        resultPath = resultPath.advanceSingle('').path;
      } else if (
        (isNodeTag(tag_) || Tags.startsNode(tag_)) &&
        (lastPropPath?.type !== Property ||
          propertyIsFull(lastPropPath.tag) ||
        ...
        if (lastPropPath.tag.value.tags[1].length < 2) {
          resultPath = resultPath.advanceSingle('').path;
        }
        if (lastPropPath.tag.value.tags[1].length < 3) {
          resultPath = resultPath.advanceSingle('').path;
        }

        if (tag_.type === TreeNode) {
          resultPath = resultPath.advanceSingle(printSums(Tags.getSums(tag_.value.children))).path;
        } else {
          resultPath = resultPath.advanceSingle('').path;
        }
      }
    ...
    switch (tag_.type) {
      ...
      case TreeNode: {
        ...
        if (node.type === TreeNode) {
          propTagPath.type === SumsTag &&
            propTagPath.tag !== printSums(Tags.sumValues(node.value.children[1]));
          if (
        ...
        let tags = BList.fromValues([firstTag, bindingTags, hashTag, statsTag, node], 1);
        let property = buildPropertyTag(tags, shift);
