...
export const buildProperty = (tags, shift) => {
  ...
  let { 0: ref, 1: bindings, 2: hash, 3: node } = tags[1];
...
export const Path = class AgastPath {
  ...
  getNodeTagPath(childrenIndex) {
    ...
    let absChildrenIndex =
      childrenIndex < 0 ? Tags.getSize(children) + childrenIndex : childrenIndex;
    return this.tagPathAt(absChildrenIndex + 1, [3]);
  }
    ...
    while (path) {
      ...
      if (!held && !shift) {
        ...
        if (nextShift) {
          ...
          tags_ = BList.fromValues([newFirstTag, tags_[1][1], tags_[1][2], tags_[1][3]], 1);
      ...
      if (!held && !shift) {
        ...
        if (nextShift) {
          ...
          tags_ = BList.fromValues(
            [newFirstTag, tags_[1][1] || '', tags_[1][2] || '', tags_[1][3]],
            1,
          );
  ...
  get held() {
    ...
    if (parseTagType(tags[1][0]) === ShiftTag && !tags[1][3]) {
  ...
  advance(tag) {
    ...
    } else if ([TreeNode, NullNode, GapNode].includes(tag_.type)) {
      resultPath = resultPath.advanceSingle(tag_).path;
    } else {
      if (
        (tag_.type === BindingTag || tag_.type === IdTag) &&
  ...
  advanceSingle(tag) {
    ...
    //   !(isNodeTag(tag_) || Tags.startsNode(tag_) || [BindingTag, IdTag].includes(tag_.type))
    // )
    //   throw new Error();
    ...
    switch (tag_.type) {
      case ReferenceTag: {
        ...
        let property = buildChild(
          Property,
          buildProperty(BList.fromValues([printTag(tag_, porcelain)])),
        );
      ...
      case TreeNode: {
        ...
        if (propertyIsFull(tagPath.tag)) {
          let tags = BList.fromValues(['', '', '', node], 1);
          let property = buildChild(Property, buildProperty(tags, shift));

          targetPath = this.replaceWith(buildNode(Tags.push(property, parentNode.value.tags)));
        } else {
          let { 0: firstTag, 1: bindingTags = '', 2: hashTag = '' } = tagPath.value.tags[1];
          let tags = BList.fromValues([firstTag, bindingTags, hashTag, node], 1);
          let property = buildChild(Property, buildProperty(tags, shift));
      ...
      case IdTag:
        let newProp = ![ReferenceTag, ShiftTag, BindingTag].includes(
          parseTagType(this.tagPathAt(-1, -1)),
        );
        let refPath = newProp ? null : this.tagPathAt(-1, 0);
        let propPath = newProp ? null : TagPath.from(refPath.path, refPath.tagsIndex);
...
export const findPropertyPath = (propertyIndex, property) => {
  ...
  let nodeSize = tags[1][3] != null ? 1 : 0;
  let valuesSize = BList.getValues(tags).length;
  let tagsSize = BList.getSize(tags);
  let absoluteIndex =
    propertyIndex_ < 0
      ? propertyIndex_ + refSize + bindingsSize + hashSize + nodeSize
      : propertyIndex_;

  let bindingsStop = refSize + bindingsSize;
  let hashStop = bindingsStop + hashSize;

  let idx = absoluteIndex;

  if (absoluteIndex === 0 && refSize) {
    idx = [0];
  } else if (absoluteIndex >= bindingsStop) {
    idx = [tags[1][2] && absoluteIndex < hashStop ? 2 : 3];
  } else if (tags[1][1] != null) {
    ...
    for (;;) {
      ...
      if (isObject(tag) && tag.type === Property && !wasLeaving) {
        if (propertyIndex.length === 0) {
          let { 0: ref, 1: bindings, 2: hash } = tag.value.tags[1];

          let index = ref ? 0 : bindings ? 1 : hash ? 2 : 3;
        ...
        if (propertyIndex.length === 1) {
          while (!propTag) {
            let { index, value } = propertyIndex[0];
            propertyIndex[0] = { index: index + 1, value };
            propTag = isNode(tag_) ? tag_ : parseTag(tag_);
