...
export const buildBoundsFromTags = (tags) => {
  let gapProperty = buildProperty([buildReferenceTag(), [], buildGapTag()]);
  ...
  if (firstProperty && firstProperty.value.type === TreeNode) {
    ...
    openStack = BTree.push(openStack, buildProperty([buildReferenceTag(), [], buildGapTag()]));
...
export const Path = class AgastPath extends WeakStackFrame {
  ...
  get isAtLeftSide() {
    ...
    return (
      !referenceTagPath ||
      (!!firstPropertyTagPath && referenceTagPath.propertyPath.equalTo(firstPropertyTagPath))
    );
  }
  ...
  get held() {
    ...
    if (tagPath.tag.type !== Property || !this.isAtLeftSide) return null;
  ...
  advanceUnwrapped(tag) {
    ...
    switch (tag.type) {
      ...
      case CloseNodeTag: {
        ...
        targetPath = this.replaceWith(buildNode(Tags.push(node.value.tags, tag)));
        break;
      }
