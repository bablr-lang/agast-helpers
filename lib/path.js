...
import { buildReferenceTag, buildShiftTag, finalizeNode } from './tree.js';
...
export const Path = class AgastPath extends WeakStackFrame {
  ...
  get firstPropertyTagPath() {
    let { tags } = this.node;
    let i = 1;
    while (Tags.getAt(i++, tags).type !== PropertyWrapper);
    return this.tagPathAt(i - 1);
  }
  ...
  replaceWith(node, binding = buildBinding()) {
    ...
    while (path) {
      ...
      let newFirstTag = tags[0];
      if (newFirstTag.type === ShiftTag) {
        let index = heldCount;
        let height = heldCount + (newFirstTag.value.height - newFirstTag.value.index);
        newFirstTag = buildShiftTag(index, height);
      }
      ...
      if (held) {
        let { tags, property } = Tags.getAt(targetParentIndex - 1, path.node.tags).value;

        let newFirstTag = tags[0];

        if (newFirstTag.type !== ShiftTag) throw new Error();

        newFirstTag = buildShiftTag(newFirstTag.value.index);

        let tags_ = [
          newFirstTag,
          buildChild(BindingTag, targetBinding),
          buildChild(Property, buildProperty(property.reference, targetBinding, held)),
        ];
        built.tags = Tags.replaceAt(
          targetParentIndex - 1,
          built.tags,
        ...
        for (let i = tags_[0].value.index - 1; i >= 0; i--) {
          built.tags = Tags.removeAt(targetParentIndex + i - 1, built.tags);
        }
      }
