...
export const getPropertyTagsIndex = (agAstNode, type, name, index, shiftIndex) => {
  ...
  if (ref?.isArray) {
    if (index < 0) {
      if (name) {
        index = sums.references[name] - initializerOffset + index;
      } else {
        index = sums.specialTypes[type] - initializerOffset + index;
      }
    } else if (index == null) {
      if (name) {
        index = sums.references[name] - 1 - initializerOffset;
      } else {
        index = sums.specialTypes[type] - 1 - initializerOffset;
      }
  ...
  if (ref?.flags.expression && shiftIndex != null) {
    if (shiftIndex < 0) {
      let shifts = 0;
      ...
      if (-shiftIndex > shifts + 1) return null;

      return parentIndex + shifts + shiftIndex + 1;
    } else {
      if (parentIndex + shiftIndex >= sums[ref.name]) return null;

      return parentIndex + shiftIndex;
    }
  }
  ...
  get next() {
    ...
    for (;;) {
      ...
      if (wrapperTag.type === BindingTag && lastRef.type === ShiftTag) {
        ...
        pathDesc.shiftIndex = lastRef.value.index;
    ...
    for (;;) {
      ...
      if (tag.type === PropertyWrapper) {
        if (wrapperIndex + 1 < tag.value.tags.length) {
          wrapperIndex++;
          continue;
        } else if (!wasLeaving && tag.value.tags[1].type !== InitializerTag) {
