...
export const getPropertyChildrenIndex = (agAstNode, reference) => {
  ...
  if (isArray) {
    if (refIndex == null) {
      refIndex = btree.getSum(agAstNode.properties[name]) - 1;
    }
...
export class TagPath {
  ...
  static from(path, childrenIndex) {
    let size = sumtree.getSum(path.node.children);
  ...
  get nextSibling() {
    ...
    const child =
      childrenIndex + 1 >= sumtree.getSum(path.node.children)
        ? null
  ...
  get next() {
    ...
    for (;;) {
      ...
      // over
      if (path.node && childrenIndex + 1 < sumtree.getSum(path.node.children)) {
        childrenIndex++;
        continue;
      }
      ...
      if (path.referenceIndex != null) {
        do {
          if (sumtree.getAt(path.referenceIndex + 2, path.outer.children)?.type === ShiftTag) {
            childrenIndex =
              sumtree.getSum(path.outer.children) > path.referenceIndex + 2
  ...
  get previousUnshifted() {
    ...
    for (;;) {
      ...
      if (tag.type === EmbeddedNode && !wasLeaving) {
        path = path.push(tag.value, childrenIndex - 1);
        childrenIndex = sumtree.getSum(tag.value.children) - 1;
        continue;
      }
