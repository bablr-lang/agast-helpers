...
const splice_ = (idx, removeCount, insertValues, tree) => {
  ...
  for (let d = depth; d > 0; d--) {
    ...
    let trailingPulldowns = [];
    {
      ...
      // pulldown
      // get values we need to have in our working space
      if (isLeaf) {
        ...
        for (let i = 0; i < values.length; i++) {
          ...
          let valueEndIdx = valueStartIdx + getSize(value) - 1;
          idx = valueEndIdx;

          if (inRange(idx_, valueStartIdx, valueEndIdx)) {
            if (idx_ !== valueStartIdx) {
              if (isLeaf) {
                leadingPulldowns.push(value);
              } else {
                leadingPulldowns.push(...value.slice(0, idx_ - valueStartIdx));
              }
            }
          }
