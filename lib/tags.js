    ...
    (acc, val) => {
      ...
      if (isArray(val)) {
        ...
        for (let i = 0; i < valRefs.length; i++) {
          let key = valRefs[i];
          let value = valRefs[++i];
          let reference = parseTag(valRefs[++i]).value;

          let record = refs.get(key);
          let count = record?.[1] ?? 0;

          if (referenceIsSingular(reference) && count > 0) {
            throw new Error('doubled reference: ' + printReference(reference));
          } else if (count > 0 && record?.[2] !== printReference(reference)) {
            throw new Error();
          }

          refs.set(key, [key, count + value, printReference(reference)]);
        }
      } else if (val) {
        ...
        if (tag.type === LiteralTag) {
          ...
          if (refOrShiftTag?.type !== ShiftTag) {
            ...
            } else if (count > 0 && record?.[2] !== printed) {
              throw new Error();
            }

            refs.set(statsRef, [statsRef, count + 1, printed]);
          }
        }
      }
      return acc;
