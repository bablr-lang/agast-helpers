...
import { buildReferenceTag, buildSumsTag } from './builders.js';
...
import { arrayValues, concat, evaluateReturn } from './iterable.js';
...
export const create = (flags = '') => {
  return treeFromValues([], flags);
};

const __treeFromValues = (values, flags, depth) => {
  ...
  let sums = sumValues(values, depth_);

  return freezeRecord([
    countValues(values, depth_),
    freezeRecord(values),
    sums,
    type,
    depth_,
...
export const treeFromValues = (values, flags = '', depth = Infinity) => {
  let depth_ = depth ?? Infinity;
  if (flags && !isString(flags)) throw new Error();
    ...
    do {
      ...
      for (let value of isArray(values) ? arrayValues(values) : values) {
        ...
        if (hashesBreak(lh3, lh2, lh1, hash)) {
          let tree = __treeFromValues(temp, flags, depth_);
          validNodes.add(tree);
          newValues.push(tree);
          temp = [];
          lh3 = lh2;
...
const setValuesAt = (idx, node, value) => {
  ...
  newValues[idx] = value;
  return treeFromValues(newValues, getDepth(node));
};
...
const ___splice = (idx, removeCount, insertValues, tree) => {
  ...
  return nextInserts.length === 1 && isArray(nextInserts[0])
    ? treeFromValues(nextInserts[0])
    : treeFromValues(nextInserts.map((node) => treeFromValues(node)));
};
...
export const buildSums = (node) => {
  let sums = getSums(node);
  if (sums) return sums;

  return sumValues(node[1]);
};
...
export const sumValues = (values, depth = null) => {
  ...
  if (depth_ > 1) {
    refCounts = [...mapSums[0].values()].sort((a, b) => compareNames(a[0], b[0])).flatMap((_) => _);
  }

  let sums = freezeRecord([refCounts, gearHashValues(values), mapSums[2], mapSums[3]]);
  return sums;
};
...
export const push = (tag, tags) => {
  ...
  if (tag_.type === OpenNodeTag) {
    ...
    let tree = __treeFromValues([tags[1][0], tag], flags, 1);
    validNodes.add(tree);
    return tree;
  } else {
    ...
    let children = getValues(tags)[1] ?? treeFromValues([], flags);

    // sometimes this is the top level
    if (sigilTag) {
      if (tag_.type === CloseNodeTag) {
        let { 0: open, 1: children = treeFromValues([], flags) } = getValues(tags);

        return treeFromValues([open, children, tag], flags, 1);
      } else {
        let { 0: open } = getValues(tags);

        children = __splice(-1, 0, [tag], children);

        return treeFromValues([open, children], flags, 1);
      }
    } else {
      return __splice(-1, 0, [tag], tags);
    }
...
export const replaceAt = (idx, tag, tree) => {
  ...
  if (parseTagType(sigilTag) === OpenNodeTag) {
    ...
    return treeFromValues(newTags, flags, 1);
  } else {
...
export const map = (fn, tree) => {
  ...
  return treeFromValues(newValues, flags, 1);
};
...
const printValue = (value) => {
  if (isArray(value)) {
    return `__:##${printHash(buildSums(value)[2])}##<//>`;
