...
const ___splice = (idx, removeCount, insertValues, tree) => {
  ...
  let remainingRemoves = removeCount;
  let remainingInserts = [...arrayValues(insertValues || [])];
  let nextRemoves = 1;
  let nextInserts = [];
  let isLeaf = true;
  ...
  for (let dPath = findTreePath(idx_, tree); dPath; dPath = dPath.parent) {
    ...
    if (isLeaf || (isFinite(path.index) && path.index > 0)) {
      // leading pulldown
      let chunk = [...listValues(path.node)].slice(0, path.index);

      if (chunk.length) {
        remainingInserts.unshift(...chunk);
        ++nextRemoves;
      }
    } else {
      if (path.parent?.index > 0) {
        // pull down the whole last chunk in case we need to re-break with it
        remainingInserts.unshift(...listValues(getValues(path.parent.node)[path.parent.index - 1]));
        ++nextRemoves;
        let { node, depth, index } = path;
        path = path.parent;
        path = freezeRecord({ ...path, index: path.index - 1 });

        path = freezeRecord({ parent: path, depth, node, index });
      }
    }
    ...
    let lastValue;
    for (let value of remainingInserts) {
      hash = stepGearHashValue(value, hash);
    ...
    if (isFinite(path.index)) {
      let index = path.index + remainingRemoves;

      path = index < path.node[1].length ? freezeRecord({ ...path, index }) : null;
    }

    let remainingRebreaks = 16;
    for (; path && isFinite(path.index); path = getNextTreePath(path)) {
      let value = path.node[1][path.index];
      let lastValue = lastPath && lastPath.node[1][path.index];
    ...
    //   // trailing pulldowns
    //   nextInserts.push(treeFromValues([...listValues(path.node)].slice(path.index)));
    // }

    remainingRemoves = nextRemoves;
    remainingInserts = nextInserts;
    nextRemoves = 1;
    nextInserts = [];
    isLeaf = false;
  }

  return remainingInserts.length === 1 ? remainingInserts[0] : treeFromValues(remainingInserts);
};
...
//     isLeaf = false;
//     insertValues_ = nextInserts;
//     removeCount_ = nextRemoves;
//   }
...
export const replaceAt = (idx, tag, tree) => {
  ...
  if (sigilTag && sigilTag.type === OpenNodeTag) {
    ...
    if (idx_ === 0) {
      ...
      let children_ = __splice(idx_ - 2, 1, [tag_], children);
