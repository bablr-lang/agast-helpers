...
import { printAttributeDefinition, printReference, printTag } from './print.js';
...
export const treeFromValues = (values, depth) => {
  ...
  let isRoot = (isString(values[1]) && startsNode(parseTag(values[1]))) || values[1] === null;
...
export const getIndex = (pathSegment, tags) => {
  if (nodeIsRoot(tags) && !tags[1][2]) return null;
  ...
  let sums = getSums(nodeIsRoot(tags) ? tags[1][2] : tags);
...
export const valuesAreNodeRoot = (values) => {
  return values[1] === null || (isString(values[1]) && startsNode(values[1]));
};
...
export const getAt = (idx, tags) => {
  if (isArray(idx)) {
    ...
    if (
      (tags[1][1] === null || parseTagType(tags[1][1]) === OpenNodeTag) &&
...
export const getSize = (tags) => {
  ...
  if (sigilTag === null || (isString(sigilTag) && parseTagType(sigilTag) === OpenNodeTag)) {
    return getSize_(tags) + getSize_(tags[1][2]) - (tags[1].length >= 3);
  } else {
...
export const map = (fn, tree) => {
  ...
  let newValues = [hash ? fn(hash) : null];
  if (open) {
