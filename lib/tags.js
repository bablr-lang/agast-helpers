  ...
  if (isArray(values)) {
    ...
    if (depth_ > 1) {
      ...
      for (let value of arrayValues(values)) {
        size += getSize(value);
        if (lastValue !== null && !valuesBreak(concat(listValues(lastValue), listValues(value)))) {
...
const valuesBreak = (values) => {
  ...
  for (let value of isArray(values) ? arrayValues(values) : values) {
    hash = stepGearHashValue(value, hash);
    if (hashesBreak(lastHash, hash)) return true; // 60845 51463 14481 6053
    lastHash = hash;
  }
  return false;
};
...
const splice_ = (idx, removeCount, insertValues, tree) => {
  ...
  for (let d = depth; d > 0; d--) {
    ...
    let leadingPulldowns = [];
    let trailingPulldowns = [];
    {
      let idx = 0;

      if (isLeaf || d === 1) {
        leadingPulldowns = values.slice(0, target + 1);
        trailingPulldowns = values.slice(target + 1 + removeCount_);
      } else {
        let started = false;
        let finished = false;
        nextRemoveCount = 0;
        for (let i = 0; i < values.length; i++) {
          let value = values[i];
          let valueStartIdx = idx;
          let valueEndIdx = valueStartIdx + getSize(value);
          idx = valueEndIdx;

          if (inRange(idx_, valueStartIdx + 1, valueEndIdx - 1)) {
            started = true;
            leadingPulldowns.push(...[...listValues(value)].slice(0, idx_ - valueStartIdx));
          }
          ...
          let targetIndex = idx_ + removeCount_ + 16;
          if (i === values.length - 1 || inRange(targetIndex, valueStartIdx, valueEndIdx)) {
            if (idx_ !== valueEndIdx) {
              trailingPulldowns.push(...[...listValues(value)].slice(idx_ - valueStartIdx));
            }
            if (!started) {
              nextRemoveCount++;
            }
    ...
    for (let i = 0; i < insertValues_.length; i++) {
      ...
      let lastValue = insertValues_[i - 1];
      ...
      if (
        !isLeaf &&
        lastValue &&
        !valuesBreak(concat(listValues(arrayLast(chunk)), listValues(value)))
      ) {
        let mergedValues = treeFromValues([
          ...concat(listValues(arrayLast(chunk)), listValues(value)),
        ]);
        chunk.pop();
