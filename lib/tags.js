  ...
  if (isArray(values)) {
    ...
    if (depth_ > 1) {
      ...
      for (let value of arrayValues(values)) {
        size += getSize(value);
        if (lastValue && !valuesBreak(concat(listValues(lastValue), listValues(value)))) {
...
const splice_ = (idx, removeCount, insertValues, tree) => {
  ...
  for (let d = depth; d > 0; d--) {
    ...
    for (let i = 0; i < insertValues_.length; i++) {
      ...
      if (!isLeaf && lastValue && !valuesBreak(concat(listValues(lastValue), listValues(value)))) {
        ...
        treeFromValues([...chunk]); // TODO remove me
      } else {
        chunk.push(value);
