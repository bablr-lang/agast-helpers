...
import { freeze, isArray, isObject, isString } from './object.js';
...
const getAt = (idx, tags) => {
  let firstType = parseTagType(module_.getAt(0, tags));
  if (isArray(idx) && idx.length > 1) {
    return module_.getAt(idx.slice(1), module_.getAt(idx.slice(0, 1), tags));
  } else {
