...
import { getSigilTag } from './path.js';
...
const splice_ = (idx, removeCount, insertValues, tree) => {
  ...
  for (let d = depth; d > 0; d--) {
    ...
    insertValues_ = [...leadingPulldowns, ...insertValues_, ...trailingPulldowns];

    if (
      insertValues_.length > 1 &&
      !isLeaf &&
      insertValues_.every((value) => value[1].length === 1) &&
      !valuesBreak([...flatMap((value) => listValues(value), insertValues_)])
    ) {
      insertValues_ = insertValues_.map((value) => value);
    }

    if (insertValues_.length > 1 && !valuesBreak(insertValues_)) {
      nextInsertValues = [treeFromValues(insertValues_)];
      nextRemoveCount = 1;
    } else {
      let lastHash = null;
      let hash = 0;
      ...
      for (let value of insertValues_) {
        ...
        if (breaks) {
          ...
          if (isLeaf) {
            ...
            let mergedValues = treeFromValues([
              ...concat(listValues(lastValue), listValues(value)),
            ]);
            chunk.pop();
            chunk.push(mergedValues);
            lastValue = mergedValues;
          }
        }
