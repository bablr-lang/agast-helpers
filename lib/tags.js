    ...
    (acc, val) => {
      ...
      if (isArray(val)) {
        ...
        for (let i = 0; i < valRefs.length; i++) {
          let ref = valRefs[i];
          let value = valRefs[++i];
          let fullRef = valRefs[++i];

          let record = refs.get(ref);
          let count = record?.[1] ?? 0;

          if (referenceIsSingular(parseTag(fullRef).value) && count > 0) {
            throw new Error('doubled reference: ' + ref);
          } else if (count > 0 && record?.[2] !== fullRef) {
            throw new Error();
          }

          refs.set(ref, [ref, count + value, fullRef]);
        }
      } else if (val) {
        ...
        if (tag.type === LiteralTag) {
          ...
          if (refOrShiftTag?.type !== ShiftTag) {
            ...
            let record = refs.get(statsRef);
            let count = record?.[1] ?? 0;
            let printed = printReference(ref);
            let fullRef = printReference({
              ...ref,
              flags: { ...parseReferenceFlags(''), array: ref.flags.array },
            });
            ...
            refs.set(statsRef, [statsRef, count + 1, fullRef]);
          }
        }
      }
      return acc;
