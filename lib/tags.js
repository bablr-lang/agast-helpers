  ...
  while (frame) {
    ...
    for (let value_ of inserts) {
      ...
      if (breaks) {
        ...
        chunk.push(isLeaf ? lastValue : treeFromValues(lastValue));
        chunk.push(isLeaf ? value : treeFromValues(value));
      } else {
        if (isLeaf || !chunk.length) {
          let value_ = isLeaf ? value : treeFromValues(value);
          ...
          let tree = treeFromValues(concat(arrayValues(lastValue), arrayValues(value)));
    ...
    if (chunk.length) {
      if (!isLeaf && !nextInserts.length) {
        ...
        if (leftSiblings && !valuesBreak(concat(leftSiblings, chunk))) {
          ...
          throw new Error('untested');
          debugger;
          // we are done and return a flatter list
        }
...
export const sumValues = (values, depth) => {
      ...
      if (isArray(val)) {
        ...
        if (tag.type === LiteralTag) {
          ...
          if (refOrShiftTag?.type !== ShiftTag) {
            let ref = refOrShiftTag || buildReferenceTag();
            let sumsRef = printSumsReferenceTag(ref);
            let record = refsSums.get(sumsRef);
            let count = record?.[1] ?? 0;
            let printed = printReferenceTag(ref);
            ...
            // refsSums.set(sumsRef, [printTag(ref), count + 1]);
          }
        }
      }
      return acc;
  ...
  let sums = freezeRecord([
    depth_ > 1
      ? [...mapSums[0].values()].sort((a, b) => compareNames(a[0], b[0])).flatMap((_) => _)
      : 0,
    gearHashValues(values),
    mapSums[2],
    mapSums[3],
  ]);
  return sums;
...
const printValue = (value) => {
  if (isArray(value)) {
    return `__:##${printHash(getSums(value)[1])}##<//>`;
    ...
    for (let tag of traverse(property.value.tags)) {
      switch (parseTagType(tag)) {
        ...
        case GapNode:
          ...
          break;
        case TreeNode:
          printed += `##${printHash(getSums(tag.value.children)[1])}##<//>`;
          break;
        case IdTag: // yes? no?
          break;
        default:
