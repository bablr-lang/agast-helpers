...
const treeFromValues = (values, depth) => {
  ...
  if (isArray(values)) {
    ...
    if (depth_ > 1) {
      ...
      for (let leafNode of leafNodes(values)) {
        if (
          lastLeafNode &&
          !hashesBreak(
            parseInt(lastLeafNode[2].gearHash, 16),
            stepGearHashValue(leafNode[1][0], parseInt(lastLeafNode[2].gearHash, 16)),
          )
        ) {
          throw new Error('unjoined node');
        }
  ...
  if (
    isArray(values) &&
    values.length <= (isString(values[1]) && startsNode(parseTag(values[1])) ? 4 : 1)
  ) {
    let stats = sumValues(values, depth_);
...
const push_ = (value, tree) => {
  ...
  for (;;) {
    ...
    if (!broken) {
      ...
      node = treeFromValues([...listValues(node), pushout]);
      pushout = null;
    }
    values = getValues(node);
  }

  result = __treeFromValues([...listValues(tree), value]);
...
const removeAt_ = (idx, tree) => {
  ...
  let leafIdx = path.length - 1;
  let returnValue = path[leafIdx].node[path[leafIdx].index];
  let isLeaf = true;
  do {
    let { node, index } = path.pop();
    let values = slice.call(getValues(node));

    if (getSize(returnValue) && !isLeaf) {
    ...
    while (idx > 0 && count < Math.min(valuesLength, index + 16)) {
      let leftHash = parseInt(getSums(values[idx - 1]).gearHash, 16);
      let rightValue = getValues(values[idx])[0];

      if (!hashesBreak(leftHash, gearHash(encodeUTF8(codePoints(printValue(rightValue)))))) {
        values[idx - 1] = treeFromValues([
          ...listValues(values[idx - 1]),
          ...listValues(values[idx]),
        ]);
        values.splice(idx);
      ...
      count += getValues(values[idx]).length;
      idx += getValues(values[idx]).length;
    }
  ...
  return returnValue[1].length === 1 ? returnValue[1][0] : returnValue;
};
...
export const removeAt = (idx, tree) => {
  ...
  if (sigilTag && sigilTag.type === OpenNodeTag) {
    ...
    if (idx_ === 0 || (isArray(idx) && idx[0] === 0)) {
      ...
      children = removeAt_(idx__, children);
