      ...
      if (isArray(val)) {
        ...
        for (let { 0: arr, 1: map } of arrs) {
          for (const { 0: key, 1: value, 2: reference } of arr) {
            ...
            if (!reference.isArray && referenceIsSingular(reference) && count > 0)
              throw new Error();
        ...
        if (val.type === LiteralTag) {
          ...
          if (refOrShiftTag.type === ReferenceTag) {
            let ref = refOrShiftTag.value;
            const { type, name } = ref;
            if (name) {
              let count = references.get(name)?.[1] ?? 0;

              if (!ref.isArray && referenceIsSingular(ref) && count > 0) throw new Error();
