...
import { compareNames, printReference, printSumsReference, printTag } from './print.js';
...
const __findSums = (ref, sortedArray, startIdx, endIdx) => {
  ...
  if (direction === 0) {
    ...
    if (direction > 0) {
      return __findSums(ref, sortedArray, idx + 1, endIdx);
    } else {
      return __findSums(ref, sortedArray, startIdx, idx - 1);
    }
  }
};

export const findSums = (ref, sortedArray, startIdx = 0, endIdx = sortedArray.length / 3 - 1) => {
  return __findSums(ref, sortedArray, startIdx, endIdx);
};
...
export const getIndex = (pathSegment, tags) => {
  ...
  if (ref?.flags.array) {
    if (index < 0) {
      index = findSums(ref, sums.refsSums) + index;
    } else if (index == null) {
      index = findSums(ref, sums.refsSums) - 1;
...
const __getPropertyTagsIndex = (tags, type, name, index) => {
  ...
  outer: while (node) {
    ...
    let valueNameCount = findSums(ref, sums.refsSums);
    ...
    for (const value of listValues(node)) {
      if (!isNode(value)) {
        ...
        if (nameCount + findSums(ref, valueSums.refsSums) > index) {
          node = value;
          continue outer;
        } else {
          nameCount += findSums(ref, valueSums.refsSums) ?? 0;
          idx += getSize(value);
        }
      }
    }
...
export const valuesAreNodeRoot = (values) => {
  return values[0] === '' || (isString(values[0]) && startsNode(values[0]));
};
...
export const sumValues = (values, depth) => {
  ...
  let mapSums = arrayReduce(
    ...
    (acc, val) => {
      ...
      if (isArray(val)) {
        ...
        let valRefs = getSums(val).refsSums;

        for (let i = 0; i < valRefs.length; i++) {
          let ref = valRefs[i];
          let value = valRefs[++i];
          let isArray = valRefs[++i];
          ...
          } else if (count > 0 && refsSums.get(ref)?.[2] !== isArray) {
            throw new Error();
          }

          refsSums.set(ref, [ref, count + value, isArray]);
        }
      } else if (val) {
        ...
        if (tag.type === LiteralTag) {
          ...
          if (refOrShiftTag?.type !== ShiftTag) {
            ...
            let sumsRef = printSumsReference(ref);
            let record = refsSums.get(sumsRef);
            let count = record?.[1] ?? 0;
            let printed = printReference(ref);

            if (referenceIsSingular(ref) && count > 0) {
              throw new Error('doubled reference: ' + printed);
            }

            refsSums.set(sumsRef, [sumsRef, count + 1, !referenceIsSingular(ref)]);
          }
        }
      }
      return acc;
  ...
  let sums = freezeRecord({
    gaps,
    lineBreaks,
    refsSums: freezeRecord(
      [...mapSums.refsSums.values()].sort((a, b) => compareNames(a[0], b[0])).flatMap((_) => _),
    ),
...
export const getSize = (tags) => {
  ...
  if (sigilTag === '' || (isString(sigilTag) && parseTagType(sigilTag) === OpenNodeTag)) {
...
export const getSigilTag = (tags) => {
  ...
  return startsNode(candidate) ? parseTag(candidate) : candidate === '' ? parseTag('<__>') : null;
};
...
export const push = (tag, tags) => {
  ...
  if (tag_.type === OpenNodeTag || (tag_.type === EmptyTag && tags[0] === 0)) {
    ...
    if (sigilTag) {
      if (tag_.type === CloseNodeTag || (tag_.type === EmptyTag && tags[0])) {
...
export const replaceAt = (idx, tag, tree) => {
  ...
  if (sigilTag === '' || parseTagType(sigilTag) === OpenNodeTag) {
