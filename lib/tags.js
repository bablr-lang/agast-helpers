...
const __treeFromValues = (values, flags, depth) => {
  ...
  let sums = depth_ > 1 ? sumValues(values, flags, depth_) : null;
...
export const getSums = (node) => {
  ...
  if (isFinite(node[0]) && node[2]) {
    return node[2];
  }

  return freezeRecord([0, 0, 0, 0]);
};
...
export const buildSums = (node) => {
  ...
  return sumValues(node[1], node[5]);
};
...
export const countRefsInValues = (values, flags = '') => {
...
export const buildRefCounts = (node) => {
  ...
  if (refCounts) {
    // if (!isArray(refCounts) || !isFrozen(refCounts)) throw new Error();
    return refCounts;
  } else {
    return countRefsInValues(node[1], getFlags(node));
  }
};

export const countGapsInValues = (values) => {
  let count = 0;
  for (let child of arrayValues(values)) {
    if (child.type === Property && child.value.node) {
...
export const buildGapsCount = (node) => {
  if (node[2]) {
    return node[2][3];
  } else {
    countGapsInValues(node[1]);
  }
};

export const countLinesInValues = (values) => {
  let count = 0;
  for (let child of arrayValues(values)) {
    ...
    if (child.type === Property && child.value.node?.type === TreeNode) {
...
export const buildLinesCount = (node) => {
  if (node[2]) {
    return node[2][2];
  } else {
    return countLinesInValues(node[1]);
  }
};

export const sumValues = (values, flags = '', depth = null) => {
  let depth_ = depth || (values.length ? getDepth(values[0]) + 1 : 1);

  let refCounts = 0;

  if (!values.length || !startsNode(values[0])) {
    refCounts = countRefsInValues(values, flags);
  }

  return freezeRecord([
    refCounts,
    gearHashValues(values),
    countLinesInValues(values),
    countGapsInValues(values),
  ]);
};
    ...
    for (let tag of BList.traverse(property.value.tags)) {
      switch (parseTagType(tag)) {
        ...
        case GapNode:
          ...
          break;
        case TreeNode:
          printed += `##${printHash(buildSums(tag.value.children)[1])}##<//>`;
          break;
        case HashTag: // yes? no?
          break;
        default:
