...
import { printAttributeDefinition, printTag } from './print.js';
import { arrayValues, concat, flatMap } from './iterable.js';
...
const treeFromValues = (values, depth) => {
  ...
  if (isArray(values)) {
    ...
    if (isRoot) {
      ...
      if (children && getFirstPropertyChild(children)?.value.shift) {
        throw new Error();
      }
    }
    ...
    if (depth_ > 1) {
      ...
      for (let value of arrayValues(values)) {
        size += getSize(value);
        if (lastValue && !valuesBreak_(concat(listValues(lastValue), listValues(value)))) {
...
const valuesBreak = (values) => {
  ...
  let lastValue = null;
  let first = true;
  let isLeaf = !values.length || (isArray(values[0]) && values[0][0] === 0) || isString(values[0]);
  for (let value of arrayValues(values)) {
    hash = stepGearHashValue(value, hash);
    if (
      isLeaf
...
export const __spliceNew = (idx, removeCount, insertValues, tree) => {
  let idx_ = !isArray(idx) && idx < 0 ? Math.max(getSize(tree) + idx + 1, 0) : idx;
  let remainingRemoveCount = removeCount;
  let remainingInsertValues = insertValues;
  let nextRemoveCount = 0;
  let nextInsertValues = [];
  let isLeaf = true;
  let path = findTreePath(idx_, tree);

  for (; path.depth > 0; path = path.parent) {
    let lastPath = null;
    let hash = 0;
    let lastHash = null;
    ...
    if (path.index > 0) {
      // leading pulldown
      let chunk = [...listValues(path.node)].slice(0, path.index);
    ...
    let chunk = [];
    let remainingRebreakCount = 16;
    for (; path; path = getNextTreePath(path)) {
      let value = path.node[1][path.index];
      let lastValue = lastPath.node[1][path.index];

      if (!remainingRemoveCount) {
        hash = stepGearHashValue(value, hash);

        nextRemoveCount += lastPath && !pathsSameNode(lastPath, path) ? 1 : 0;
      ...
      if (remainingRemoveCount) {
        --remainingRemoveCount;
      } else if (remainingRebreakCount) {
        --remainingRebreakCount;
      } else {
        break;
      }
    ...
    if (path && path.index < path.node[1].length) {
      // trailing pulldowns
      nextInsertValues.push(treeFromValues([...listValues(path.node)].slice(path.index)));
    }
    remainingRemoveCount = nextRemoveCount;
    nextRemoveCount = 1;
    nextInsertValues = [];
    isLeaf = false;
  }

  return remainingInsertValues.length === 1
    ? remainingInsertValues[0]
    : treeFromValues(remainingInsertValues);
};
...
export const __splice = (idx, removeCount, insertValues, tree) => {
  ...
  let insertValues_ = [...arrayValues(insertValues)];
  let removeCount_ = removeCount;

  if (isArray(idx_)) {
    idx_ = indexFromPath(path);
  }
  ...
  for (let d = depth; d > 0; d--) {
    ...
    let leadingPulldowns = values.slice(0, target + 1);
    let trailingPulldowns = values.slice(target + 1 + removeCount_);
    let lastInsertValues = insertValues_;

    insertValues_ = [...leadingPulldowns, ...lastInsertValues, ...trailingPulldowns];
    ...
    let i = -1;
    for (let value of insertValues_) {
      i++;
      hash = stepGearHashValue(value, hash);
      ...
      if (breaks) {
        if (
          !nextInsertValues.length ||
          valuesBreak_(concat(listValues(arrayLast(nextInsertValues)), chunk))
        ) {
          nextInsertValues.push(treeFromValues(chunk));
          if (!false) {
            nextRemoveCount++;
          ...
          let lastInsertValue = nextInsertValues.pop();
          nextInsertValues.push(treeFromValues(concat(listValues(lastInsertValue), chunk)));
        }
        ...
        lastHash = null;
        lastValue = null;
        treeFromValues([...chunk]);
      } else {
        if (isLeaf) {
          chunk.push(value);
          lastValue = value;
        } else {
          let mergedValues = treeFromValues([...concat(listValues(lastValue), listValues(value))]);
          chunk.pop();
          chunk.push(mergedValues);
          lastValue = value;
        }
      }
  ...
  return insertValues_.length === 1 ? insertValues_[0] : treeFromValues(insertValues_);
};
...
const __getPropertyTagsIndex = (tags, type, name, index) => {
  let nameCount = 0;
  let node = isString(tags[1][1]) && startsNode(parseTag(tags[1][1])) ? getValues(tags)[2] : tags;
  let idx = -1;
...
export const replaceAt = (idx, tag, tree) => {
  ...
  if (sigilTag && sigilTag.type === OpenNodeTag) {
    ...
    if (idx_ === 0) {
      ...
      children = __splice(idx_ - 2, 1, [tag_], children);
