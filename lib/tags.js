...
const fromValues = (...args) => {
  return fromValues_(...args);
};

const getAt = (idx, tags) => {
  if (isArray(idx)) {
    if (idx.length > 1) {
      let outer = module_.getAt(arraySlice(idx, 0, 1), tags);

      return outer
        ? module_.getAt(arraySlice(idx, 1), isArray(outer) ? outer : outer.value.tags)
        : null;
    } else {
    ...
    if (
      ...
      return module_.getAt(absIdx - 2, tags[1][2]);
    } else {
      if (isNode(tags) && tags[1][1] && parseTagType(tags[1][1]) === OpenNodeTag) {
        return module_.getAt([absIdx >= 2 ? absIdx - getSize(tags[1][2]) + 1 : absIdx], tags);
      } else {
        return module_.getAt(idx, tags);
      }
    }
...
const push = (tag, tags) => {
  ...
  if (isNode(tag_)) {
    ...
    let children = getValues(tags)[2] ?? module_.fromValues([]);

    let { 0: hash, 1: open } = getValues(tags);

    children = module_.concat(children, tag_);

    return module_.fromValues([hash, open, children]);
  } else {
    if (tag_.type === OpenNodeTag) {
      ...
      if (sigilTag) {
        if (tag_.type === CloseNodeTag) {
          let { 0: hash, 1: open, 2: children = module_.fromValues([]) } = getValues(tags);

          return module_.fromValues([hash, open, children, printTag(tag_)], 1);
        } else {
          let { 0: hash, 1: open } = getValues(tags);

          children = module_.push(printTag(tag_), children);

          return module_.fromValues([hash, open, children], 1);
        }
      } else {
...
const replaceAt = (idx, tag, tree) => {
  ...
  if (sigilTag && sigilTag.type === OpenNodeTag) {
    ...
    if (idx_ === 0) {
      ...
      children = module_.replaceAt(idx_ - 2, tag_, children);

      newTags[2] = children;
    }
    return module_.fromValues(newTags, 1);
  } else {
...
const removeAt = (idx, tree) => {
  ...
  if (sigilTag && sigilTag.type === OpenNodeTag) {
    ...
    if (idx_ === 0) {
      ...
      children = module_.removeAt(idx_ - 2, children);
...
const map = (fn, tree) => {
  ...
  let newValues = [];
  if (hash) newValues.push(fn(hash));
  if (open) newValues.push(fn(open));
  if (children) newValues.push(module_.map(fn, children));
  if (close) newValues.push(fn(close));

  return module_.fromValues(newValues, 1);
};
