...
export const __splice = (idx, removeCount, insertValues, tree) => {
  ...
  let remainingInserts = insertValues;
  let nextRemoves = 0;
  let nextInserts = [];
  let isLeaf = true;
  ...
  for (let dPath = findTreePath(idx_, tree); dPath; dPath = dPath.parent) {
    ...
    if (isFinite(path.index) && path.index > 0) {
      // leading pulldown
      ...
      //   requires us to adjust `index` backwards for path.parent
    }
...
export const replaceAt = (idx, tag, tree) => {
  ...
  if (sigilTag && sigilTag.type === OpenNodeTag) {
    ...
    if (idx_ === 0) {
      ...
      if (children_[0] <= children[0]) throw new Error();

      newTags[2] = children_;
    }
    return treeFromValues(newTags, 1);
  } else {
    let tree_ = __splice(idx_, 1, [tag_], tree);

    if (tree_[0] <= tree[0]) throw new Error();
