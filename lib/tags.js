...
import { deepFreezeRecord, isArray, isObject, isRecord, isString } from './object.js';
...
export const sumValues = (values, depth) => {
  ...
  let stats = {
    gaps,
    lineBreaks,
    names: [...mapStats.names.values()].sort((a, b) => compareNames(a[0], b[0])).flatMap((_) => _),
    types: [...mapStats.types.values()].sort((a, b) => compareNames(a[0], b[0])).flatMap((_) => _),
  };
  return deepFreezeRecord(stats);
};
