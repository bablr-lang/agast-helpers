  ...
  if (isArray(values)) {
    ...
    if (depth_ > 1) {
      ...
      if (size && !isRoot) {
        ...
        for (let idx = 0; ; idx++) {
          let value = values[valueFirstIdx];
...
const splice_ = (idx, removeCount, insertValues, tree) => {
  let idx_ = idx < 0 ? Math.max(getSize(tree) + idx, 0) : idx;
  ...
  do {
    ...
    if (insertValues_.length) {
      values.splice(index, removeCount, ...insertValues_);
      insertValues_ = [];
      removeCount = 1;
    } else {
    ...
    if (values.length > 1) {
      ...
      let lowerBound = Math.max(index - 16, 1);
      let upperBound = Math.min(valuesLength, index + 16);
      let idx = lowerBound;
      let count = idx;

      while (count < upperBound) {
        let leftHash = gearHash16At(idx - 1, values); // TODO perf
  ...
  // return treeFromValues(insertValues_.length === 1 ? insertValues_[0] : insertValues_);
  return treeFromValues(insertValues_);
};
...
export const gearHash16At = (index, values) => {
  let start = Math.max(0, index - 16);
  let end = index;

  let hash = 0;
  for (let i = start; i <= end; i++) {
    hash = stepGearHashValue(values[i], hash);
  }
  return hash;
};
