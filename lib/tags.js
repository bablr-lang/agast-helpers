...
const NODE_SIZE = defaultNodeSize;
...
const findBalancePoint = (values) => {
  let leftSum = 0;
  let rightSum = reduce.call(values, (sum, v) => sum + getSize_(v), 0);
...
const concat = (first, second) => {
  ...
  if (firstHeight === secondHeight) {
    ...
    if (!nodeCollapses(pushout)) {
      let targetIndex = firstHeight > secondHeight ? getSize_(tree) : 0;
      return addAt(targetIndex, pushout, tree);
    } else {
      let depth = Math.abs(firstHeight - secondHeight) - 1;
      let targetIndex = firstHeight > secondHeight ? getSize_(tree) - 1 : 0;
      ...
      if (getValues(pushout).length + getValues(targetNode).length <= NODE_SIZE) {
        return replaceAt_(targetIndex, treeFromValues(combinedValues), tree);
      } else {
        let { leftValues, rightValues } = splitValues(combinedValues);

        if (firstHeight > secondHeight) {
          return replaceAt_(targetIndex, leftValues, addAt(targetIndex + 1, tree, rightValues));
          // return addAt(targetIndex + 1, replaceAt_(targetIndex, leftValues, tree), rightValues);
        } else {
          // TODO change order of operations like above?
          return addAt(targetIndex, replaceAt_(targetIndex, rightValues, tree), leftValues);
        }
      }
    }
  }
...
export const addAt = (idx, value, tree) => {
...
export const assertValidNode = (node) => {
  if (!validNodes.has(node)) throw new Error();
};
...
export const sumValues = (values, depth) => {
  ...
  let stats = freezeRecord({
    ...
    hashes: null,
    gearHash: gearHashValues(values),
  });
  return stats;
};
...
export const push = (tag, tags) => {
  ...
  if (isNode(tag_)) {
    ...
    if (tag_.type === OpenNodeTag) {
      ...
      return push_(printTag(tag_), tags);
    } else {
      if (!sigilTag || sigilTag.type !== OpenNodeTag) {
        return push_(printTag(tag_), tags);
      }
...
export const gearHash = (bytes) => {
  ...
  for (let byte of bytes) {
    hash = ((hash << 1) + GEARS[byte]) & 0xffff;
  }

  return hash;
};

export function* __gearHashValues(values) {
  let hash = 0;

  for (let value of arrayValues(values)) {
    let valueHash = gearHash(encodeUTF8(printValue(value)));

    yield (hash = ((hash << 1) + GEARS[valueHash >> 8]) & 0xffff);
  }
...
export const gearHashValues = (values) => {
  let iter = __gearHashValues(values);
  let step = iter.next();
...
const printValue = (value) => {
  if (isArray(value)) {
    ...
    for (let child of arrayValues(getValues(value))) {
      children += printValue(child);
    }
    ...
    for (let tag of BList.traverse(property.value.tags)) {
      switch (tag.type) {
        ...
        case GapNode:
          ...
          break;
        case TreeNode:
          printed += `##${printHash(getSums(tag.value.tags).gearHash)}##<//>`;
          break;
        case HashTag:
