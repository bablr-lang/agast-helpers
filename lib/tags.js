...
import {
  ...
  printReferenceTag,
  printSumsReference,
  printSumsReferenceTag,
  printTag,
} from './print.js';
import { arrayValues, concat, evaluateReturn, StreamIterable } from './iterable.js';
...
export const create = (refCountsType = 0) => {
  return treeFromValues([], refCountsType);
};

const __treeFromValues = (values, refCountsType, depth) => {
  ...
  let sums = sumValues(values, refCountsType, depth_);
  return freezeRecord([countValues(values, depth_), freezeRecord(values), sums, type, depth_]);
};
...
export const treeFromValues = (values, refCountsType = 0, depth = Infinity) => {
  let depth_ = depth ?? Infinity;
  ...
  let isRoot = (isString(values[0]) && startsNode(parseTag(values[0]))) || values[0] === '';
      ...
      for (let value of arrayValues(values)) {
        ...
        switch (tag.type) {
          ...
          case LiteralTag:
            ...
            if (lastLiteralTag && !literalBreaks(lastLiteralTag.value + tag.value))
              throw new Error();
            lastLiteralTag = tag;
            break;
        }
  ...
  if (isArray(values)) {
    ...
    tree = freezeRecord([countValues(values, depth_), freezeRecord(values), sums, type, depth_]);
    validNodes.add(tree);
  } else {
    let newValues;
    let hash = null;
    ...
    do {
      ...
      for (let value of isArray(values) ? arrayValues(values) : values) {
        ...
        if (hashesBreak(lh3, lh2, lh1, hash)) {
          let tree = __treeFromValues(temp, refCountsType, depth_);
          validNodes.add(tree);
          newValues.push(tree);
          temp = [];
          lh3 = lh2;
...
export const getRefCountsType = (node) => {
  return getSums(node)?.[0] ?? 0;
};
...
const setValuesAt = (idx, node, value) => {
  ...
  newValues[idx] = value;
  return treeFromValues(newValues, getRefCountsType(node), getDepth(node));
};
...
const ___splice = (idx, removeCount, insertValues, tree) => {
  ...
  let refCountsType = getRefCountsType(tree);
  ...
  while (frame) {
    ...
    if ((isLeaf || isFinite(target)) && target > 0) {
      // leading pulldown
      let chunk = [...listValues(frame.node)].slice(0, target);
      ...
      if (frame.parent?.path.index > 0) {
        ...
        inserts.unshift(
          ...listValues(getValues(frame.parent.node)[frame.parent.path.index - 1]).map((value) =>
    ...
    for (let value_ of inserts) {
      ...
      if (breaks) {
        ...
        chunk.push(isLeaf ? lastValue : treeFromValues(lastValue, refCountsType));
        chunk.push(isLeaf ? value : treeFromValues(value, refCountsType));
      } else {
        if (isLeaf || !chunk.length) {
          let value_ = isLeaf ? value : treeFromValues(value, refCountsType);
          ...
          let tree = treeFromValues(
            concat(arrayValues(lastValue), arrayValues(value)),
            refCountsType,
          );
    ...
    for (; path && isFinite(target) && remainingRebreaks; path = getNextTreePath(path)) {
      let value = path.node[1][path.index];
    ...
    if (chunk.length) {
      if (!isLeaf && !nextInserts.length) {
        ...
        let leftSiblings = leftSiblingsPath?.node[1][leftSiblingsPath.index];
        let rightSiblingsPath = getNextTreePath(frame.parent.path);
        let rightSiblings = rightSiblingsPath?.node[1][rightSiblingsPath.index];

        if (leftSiblings && !valuesBreak(concat(leftSiblings, chunk))) {
          throw new Error('untested');
          debugger;
          // merge with left
          path = leftSiblingsPath;
        } else if (rightSiblings && !valuesBreak(concat(rightSiblings, chunk))) {
          throw new Error('untested');
          debugger;
    ...
    if (path && (isLeaf || isFinite(target)) && path.index < path.node[1].length) {
      // trailing pulldown
      let chunk = [...listValues(path.node)].slice(path.index);
  ...
  return nextInserts.length === 1 && isArray(nextInserts[0])
    ? treeFromValues(nextInserts[0], refCountsType)
    : treeFromValues(
        nextInserts.map((node) => treeFromValues(node, refCountsType)),
        refCountsType,
      );
};
...
export const __findPath = (idx, tree, options = freezeRecord({})) => {
  ...
  stack: while (node) {
    ...
    for (
      ...
      if (isNode(value) && path.length + 1 < Math.min(depth, treeDepth)) {
        ...
        let sum = getSize_(candidateNode);
...
export const indexForPath = (path, tree) => {
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    for (let j = 0; j < frame.index; j++) {
      let value = frame.node[1][j];

      index += value ? getSize(value) : 0;
    }
  }
...
const __findCount = (ref, sortedArray, startIdx, endIdx) => {
  if (!sortedArray.length || endIdx < startIdx) return null;
  if (!(isString(ref) || isObject(ref))) throw new Error();

  let expected = printSumsReference(isString(ref) ? parseTag(ref).value : ref);
  ...
  if (direction === 0) {
    ...
    if (direction > 0) {
      return __findCount(ref, sortedArray, idx + 1, endIdx);
    } else {
      return __findCount(ref, sortedArray, startIdx, idx - 1);
    }
  }
};

export const findCount = (ref, sortedArray, startIdx = 0, endIdx = sortedArray.length / 2 - 1) => {
  return __findCount(ref, sortedArray, startIdx, endIdx);
};
...
export const getIndex = (pathSegment, tags) => {
  ...
  let counts = buildRefCounts(nodeIsRoot(tags) ? tags[1][1] : tags);

  if (ref?.flags.array) {
    if (index < 0) {
      index = findCount(ref, counts) + index;
    } else if (index == null) {
      index = findCount(ref, counts) - 1;
...
const __getPropertyTagsIndex = (tags, type, name, index) => {
  ...
  outer: while (node) {
    let counts = buildRefCounts(node);

    if (!counts) return null;

    let ref = printTag(buildReferenceTag(type, name));
    let valueNameCount = findCount(ref, counts);
    ...
    for (const value of listValues(node)) {
      if (!isNode(value)) {
        ...
        let counts = buildRefCounts(value);
        if (nameCount + findCount(ref, counts) > index) {
          node = value;
          continue outer;
        } else {
          nameCount += findCount(ref, counts) ?? 0;
          idx += getSize(value);
        }
      }
    }
...
export const nodeIsRoot = (node) => {
  return startsNode(getValues(node)[0]);
};

export const buildRefCounts = (node) => {
  let refCounts = getSums(node)[1];
  if (refCounts) {
    return refCounts;
  }

  let refsMap = new Map();
  for (let value of arrayValues(node[1])) {
    if (isArray(value)) {
      let refCounts = buildRefCounts(value);
      for (let i = 0; i < refCounts.length; i += 2) {
        let ref = refCounts[i];
        let count = refCounts[i + 1];

        // TODO check for incompatibility
        refsMap.set(ref, [ref, count + 1]);
      }
    } else {
      let tag = parseTag(value);
      ...
      if (tag.type === Property) {
        ...
        let refOrShiftTag = !tags[1][0] ? buildReferenceTag() : parseTag(tags[1][0]);

        if (refOrShiftTag?.type !== ShiftTag) {
          let ref = refOrShiftTag || buildReferenceTag();
          let sumsRef = printSumsReferenceTag(ref);
          let record = refsMap.get(sumsRef);
          let count = record?.[1] ?? 0;
          let printed = printReferenceTag(ref);

          if (referenceIsSingular(ref.value) && count > 0) {
            throw new Error('doubled reference: ' + printed);
          }

          refsMap.set(sumsRef, [printTag(ref), count + 1]);
        }
      }
    }
  }
  ...
  return freezeRecord(
    [...refsMap.values()].sort((a, b) => compareNames(a[0], b[0])).flatMap((_) => _),
  );
};

export const sumValues = (values, refCountsType, depth) => {
  if (values.length && startsNode(values[0])) {
    return null;
  }

  let depth_ = depth || (values.length ? getDepth(values[0]) + 1 : 1);

  let mapSums = arrayReduce(
    values,
    (acc, val) => {
      const { 1: refsSums } = acc;
      if (isArray(val)) {
        acc[3] += getSums(val)[3];
        acc[4] += getSums(val)[4];

        let valRefs = buildRefCounts(val);

        for (let i = 0; i < valRefs.length; i++) {
          let ref = valRefs[i];
          let value = valRefs[++i];

          let isArray = !referenceIsSingular(parseTag(ref).value);

          let record = refsSums.get(ref);
          let count = record?.[1] ?? 0;
          let recordIsArray = !!record && !referenceIsSingular(parseTag(record[0]).value);

          if (!isArray && count > 0) {
            throw new Error('doubled reference: ' + ref);
          } else if (count > 0 && recordIsArray !== isArray) {
            throw new Error();
          }

          refsSums.set(ref, [ref, count + value]);
        }
      } else if (val) {
    ...
    [refCountsType, new Map(), null, 0, 0],
  );

  let refCounts = 0;

  if (depth_ > 1) {
    refCounts = [...mapSums[1].values()].sort((a, b) => compareNames(a[0], b[0])).flatMap((_) => _);
  }

  let sums = freezeRecord([mapSums[0], refCounts, gearHashValues(values), mapSums[3], mapSums[4]]);
  return sums;
};
...
export const getSize = (tags) => {
  ...
  let sigilTag = values[0];
  if (parseTagType(sigilTag) === OpenNodeTag) {
...
export const findPath = (path, tree, options) => {
  if (!isNode(tree)) throw new Error();
  let sigilTag = getValues(tree)[0];

  let isRootNode = [EmptyTag, OpenNodeTag, NullTag, GapTag, ReferenceTag, ShiftTag].includes(
    parseTagType(sigilTag),
  );

  if (isRootNode) {
    if (isArray(path)) {
      if (path.length > 1) {
        let rootPath = __findPath(arraySlice(path, 0, 1), tree);

        if (!rootPath) return null;

        let rootValue = rootPath[0].node[1][rootPath[0].index];

        if (isNode(rootValue)) {
          let childrenSeg = __findPath(arraySlice(path, 1), rootValue);

          if (childrenSeg) {
            rootPath = freezeRecord([rootPath[0], ...arrayValues(childrenSeg)]);
          }
        }
        return rootPath;
      } else {
        return __findPath(path, tree, options);
      }
    } else {
      let idx = path;
      let idx_ = idx < 0 ? getSize(tree) + idx : idx;

      if (tree[1].length > 2 && idx_ === getSize(tree) - 1) {
        return __findPath([2], tree, options);
      } else if (idx_ === 0) {
        return __findPath(idx_, tree, options);
      } else {
        let children = getValues(tree)[1];

        let childrenPath = __findPath(idx_ - 1, children, options);

        return (
          childrenPath &&
          freezeRecord([freezeRecord({ index: 1, node: tree }), ...arrayValues(childrenPath)])
        );
      }
    }
  } else {
    return __findPath(path, tree);
  }
};

export const push = (tag, tags) => {
  if (!isNode(tags)) throw new Error();
  if (isNode(tag)) throw new Error('not implemented');

  let tag_ = parseTag(tag);
  let sigilTag = getSigilTag(tags);
  let refCountsType = getRefCountsType(tags);
  ...
  if (tag_.type === OpenNodeTag) {
    ...
    let tree = __treeFromValues([tags[1][0], tag], refCountsType);
    validNodes.add(tree);
    return tree;
  } else {
    ...
    let children = getValues(tags)[1] ?? treeFromValues([], refCountsType);

    // sometimes this is the top level
    if (sigilTag) {
      if (tag_.type === CloseNodeTag) {
        let { 0: open, 1: children = treeFromValues([], refCountsType) } = getValues(tags);

        return treeFromValues([open, children, tag], refCountsType, 1);
      } else {
        let { 0: open } = getValues(tags);

        children = __splice(-1, 0, [tag], children);

        return treeFromValues([open, children], refCountsType, 1);
      }
    } else {
      return __splice(-1, 0, [tag], tags);
    }
...
export const replaceAt = (idx, tag, tree) => {
  ...
  let tag_ = tagObj.type === Property ? tagObj : printTag(tagObj, porcelain);
  let refCountsType = getRefCountsType(tree);
  ...
  if (parseTagType(sigilTag) === OpenNodeTag) {
    ...
    return treeFromValues(newTags, refCountsType, 1);
  } else {
...
export const map = (fn, tree) => {
  ...
  return treeFromValues(newValues, refCountsType, 1);
};
...
export const gearHashValues = (values) => {
  return gearHash(UTF8From([...arrayValues(values)].map((value) => printValue(value)).join('')));
};
...
const printValue = (value) => {
  if (isArray(value)) {
    return `__:##${printHash(getSums(value)[2])}##<//>`;
  } else if (isObject(value)) {
    let property = value;
    let printed = '';
    for (let tag of BList.traverse(property.value.tags)) {
      switch (parseTagType(tag)) {
        ...
        case GapNode:
          ...
          break;
        case TreeNode:
          printed += `##${printHash(getSums(tag.value.children)[2])}##<//>`;
          break;
        case HashTag: // yes? no?
          break;
        default:
