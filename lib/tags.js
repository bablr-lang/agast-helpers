...
const ___splice = (idx, removeCount, insertValues, tree) => {
  ...
  for (let frame = splicePath; frame; frame = frame.parent) {
    ...
    if ((isLeaf || isFinite(target)) && target > 0) {
      ...
      if (frame.parent?.path.index > 0) {
        ...
        inserts.unshift(
          ...listValues(getValues(frame.parent.node)[frame.parent.path.index - 1]).map(
            (value) => value[1],
          ),
        );
        ++parentFrame.target;
    ...
    let remainingRemoves = frame.removes;
