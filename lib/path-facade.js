...
export function* allTagPathsFor(range, options = {}) {
  ...
  while (path) {
    ...
    let gapPath = path.path.parent && path.path.parent.tagPathAt(path.path.referenceIndex + 1);

    if (endPath && path.tag.type === CloseNodeTag && gapPath && endPath.equalTo(gapPath)) {
      return;
    }
