...
import { referenceFlags, parseTagType } from './builders.js';
...
export const printBinding = (binding) => {
  ...
  return `:${
    segments.length
      ? Array.prototype.map.call(segments, (segment) => printBindingSegment(segment)).join('/')
      : ''
  }:`;
};
...
export const printNullTag = (tag) => {
  if (tag && parseTagType(tag) !== NullTag) throw new Error();
