/* global btoa */
import { freezeRecord, isDeepRecord } from '@bablr/record';
import {
  DoctypeTag,
  OpenNodeTag,
  CloseNodeTag,
...
export const printRefsSums = (refsSums, options = {}) => {
  let pclSp = options.porcelain ? '' : ' ';
  ...
  for (let i = 0; i < refsSums.length; i += 3) {
    sums.push(printRefSums(refsSums[i], refsSums[i + 1], refsSums[i + 2]));
  }

  return `{${pclSp}${sums.join(`,${pclSp}`)}${pclSp}}`;
};

export const printRefSums = (ref, count, isArray, options = {}) => {
  let pclSp = options.porcelain ? '' : ' ';

  return `${printReference(ref)}${pclSp}${printExpression(count)},${pclSp}${printExpression(
    Number(isArray),
  )}`;
};

export function* printSums(sums, options = {}) {
  let pclSp = options.porcelain ? '' : ' ';
  let { refsSums, gearHash, lineBreaks, gaps } = sums;

  return `#[${printRefsSums(refsSums)},${pclSp}${printString(gearHash)},${pclSp}${printExpression(
    lineBreaks,
  )},${pclSp}${printExpression(gaps)}]#`;
}

export const printSumsTag = (tag) => {
  return printSums(tag.value);
};
...
export const printSumsReference = (ref) => {
  const { type, name } = ref;
  return printTag(buildReferenceTag(type, name));
};

export const printSumsReferenceTag = (tag) => {
  return printSumsReference(tag.value);
};
...
export const printTag = (tag, options = {}) => {
  ...
  return printer ? printer(tag, options) : tag;
};
