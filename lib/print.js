...
import { buildReferenceTag, parseObject, parseStreamTag } from './builders.js';
...
export const printArray = (arr, options = {}) => {
  ...
  return `[${[...arrayValues(arr)].map((v) => printExpression(v)).join(`,${pclSpace}`)}]`;
};
...
export const printObject = (obj, options = {}) => {
  ...
  return entries_.length
    ? `{${pclSpace}${entries_
        ...
        .map(({ 0: k, 1: v }) => `${k}:${pclSpace}${printExpression(v)}`)
        .join(`,${pclSpace}`)}${pclSpace}}`
    : '{}';
};
...
export const printStatsReference = (ref) => {
  const { type, name } = ref;
  return printTag(buildReferenceTag(type, name));
};

export const printStatsReferenceTag = (tag) => {
  return printStatsReference(tag.value);
};
