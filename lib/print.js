...
export const printOpenNodeTag = (tag) => {
  ...
  let { flags, name, literalValue, attributes, selfClosing } = tag.value;
  ...
  let typeFrag = name ? printType(name) : '';
...
export const printSelfClosingNodeTag = (tag, literalValue) => {
  ...
  return `<${printNodeFlags(flags)}${printName(type)}${intrinsicFrag}${attributesFrag} />`;
};
