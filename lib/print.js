...
export const printIOStreamOpenTag = (tag) => {
  if (tag?.type !== IOStreamOpenTag) throw new Error();
...
export const printCloseTag = (tag) => {
  if (tag?.type !== CloseTag) throw new Error();
...
export const printIOTag = (tag) => {
  ...
  if (tag.type === IOStreamOpenTag) {
    return printIOStreamOpenTag(tag);
  }
  return printTag(tag);
};
