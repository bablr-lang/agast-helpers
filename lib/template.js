...
export const interpolate = (expressions, node) => {
  ...
  outer: for (let expression of expressions) {
    ...
    do {
      path = path.atDepth(0);
      let tag;
      while ((tag = Tags.getAt(i, Tags.getTags(path.node))) != null) {
        if (path.node.type === GapNode) {
          path = path.replaceWith(expression ?? buildNullNode());
          continue outer;
        } else if (isObject(tag) && tag.type === Property && tag.value.tags[1][4]) {
          let node_ = tag.value.tags[1][4];
          if (node_.type === GapNode || (node_.type === TreeNode && node_.value.children[2][3])) {
            path = path.push(i);
            i = 0;
            continue;
          }
