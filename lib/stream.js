  ...
  for (;;) {
    ...
    if (tag.type === OpenNodeTag) {
      ...
      if (!frame?.ref || frame.open) {
        ...
        frame.heldProperties = parentFrame?.heldProperties || [];
      ...
      if (tag.value.type === '#') {
        if (nodeDepth === 1 && !parentFrame.expanded && !parentFrame.open) {
      ...
      frame = buildFrame(doneFrame.parent);
