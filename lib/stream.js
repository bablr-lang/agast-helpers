...
export const flatMap = (fn, tags) =>
  new StreamIterable(__flatMap(fn, tags))[Symbol.iterator]().next().value;
...
function* __flattenInternalFragments(tags) {
  ...
  for (;;) {
    ...
    if (tag.type === OpenNodeTag) {
      ...
      if (referencesAreEqual(tag.value, buildReference('.'))) {
        ref = tag;
        continue;
      } else {
        if (open) {
      ...
      if (open && !tag.value.languagePath) {
        bind = tag;
        continue;
      } else {
        if (open) {
