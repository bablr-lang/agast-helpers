...
import { buildBindingTag, buildOpenNodeTag } from './builders.js';
import {
  OpenNodeTag,
  CloseNodeTag,
  ReferenceTag,
...
function* __flattenInternalFragments(tags) {
  const co = new Coroutine(getStreamIterator(tags));
  let held = { opens: 0, ref: null, open: null, bind: null };
  let stack = [];
  ...
  for (;;) {
    ...
    if (tag.type === OpenNodeTag) {
      if (tag.value.flags.fragment && tag.value.flags.cover && (stack.length || held.opens > 0)) {
        if (!tag.value.selfClosing) {
          stack.push(held);
          held = { opens: 0, ref: held.ref, open: tag, languagePath: held.languagePath };
        }
        ...
        if (held.languagePath) {
          yield buildBindingTag(held.languagePath);
        }
        if (!tag.value.flags.fragment && held.open && held.opens === 0) {
          held.open = held.ref = held.languagePath = null;
        } else {
          held.ref = null;
          held.languagePath = null;
        }
        if (!tag.value.selfClosing) {
          held.opens++;
        }
      ...
      if (held.ref) {
        if (held.languagePath) {
          held.languagePath = [...held.languagePath, ...tag.value.languagePath];
        } else {
          held.languagePath = tag.value.languagePath;
        }
        continue;
      }
