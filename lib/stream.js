...
export const treeFromStream = (tags, options = freeze({})) => __treeFromStream(tags, options);

export const treeFromStreamSync = (tokens, options = freeze({})) => {
  return evaluateReturnSync(treeFromStream(tokens, options));
};

export const treeFromStreamAsync = async (tokens, options = freeze({})) => {
  ...
  for (;;) {
    ...
    if (tag.type === OpenNodeTag) {
      ...
      if (tag.value.type === '#' && !tag.value.name) {
        frame.heldProperties.push(buildProperty(Tags.fromValues([tag])));
      } else {
      ...
      if (heldProperties.length) {
        ...
        if (!node) {
          heldProperties[heldProperties.length - 1] = buildProperty(
            Tags.fromValues([tags[1][0], Tags.push(tag, tags[1][1] || Tags.fromValues([]))]),
          );
