      ...
      if (frame?.ref.type === '#' && !frame.ref.name) {
        ...
        if (heldProperties.length) {
          ...
          if (node.value.type === Symbol.for('_')) {
                ...
                frame.heldProperties[heldProperties.length - 1] = buildProperty(
                  BList.fromValues(
                    [
                      ...
                      heldProperty.tags[1][1] || Tags.empty(),
                      getRoot(node),
                    ],
                    1,
                  ),
            ...
            frame.heldProperties[heldProperties.length - 1] = buildProperty(
              BList.fromValues(
                [heldProperty.tags[1][0], heldProperty.tags[1][1] || Tags.empty(), node],
                1,
              ),
            );
          }
      ...
      if (heldProperties.length) {
        ...
        if (!node) {
          heldProperties[heldProperties.length - 1] = buildProperty(
            BList.fromValues(
              [tags[1][0] || Tags.empty(), Tags.push(printTag(tag), tags[1][1] || Tags.empty())],
              1,
            ),
          );
