    ...
    if (tag.type === OpenNodeTag) {
      ...
      if (
        ...
        if (heldProperties.length) {
          ...
          let node = yield* treeFromStream(new StreamIterable(__hoistTrivia(co)));
      ...
      if (!frame?.ref || frame.open) {
        ...
        if (isCover) {
          ...
          frame.bindings = parentFrame && tag.value.flags.cover ? [...parentFrame.bindings] : [];
          frame.shifting = parentFrame && tag.value.flags.cover ? parentFrame.shifting : false;
        } else {
      ...
      if (!lastFrame.open.value.flags.fragment || !frame) {
        for (let property of lastFrame.heldProperties) {
          yield buildChild(ReferenceTag, property.reference);
          for (let binding of property.bindings) {
            yield buildChild(BindingTag, binding);
          }
      ...
      if (frame) {
        ...
        if (lastFrame.expanded || !lastFrame.open.value.flags.cover) {
          yield tag;
        }
        break;
      }
