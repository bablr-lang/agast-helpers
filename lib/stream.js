...
import { arrayLast, freeze, isPlainObject } from './object.js';
...
export const prettyGroupTags = (tags, options = {}) =>
  new StreamIterable(__prettyGroupTags(tags, options));

function* __prettyGroupTags(tags, options) {
...
function* __streamFromTree(doctypeTag, rootNode, options) {
  let { unshift = false, getGapNode, checkBalance = true } = options;
  if (!rootNode || !Tags.getSize(Tags.getTags(rootNode))) return;

  let rootIsFragment = rootNode.value.type === Symbol.for('__');

  let tagPath = rootIsFragment ? TagPath.fromNode(rootNode, 2, 0) : TagPath.fromNode(rootNode);
  let count = 0;
...
function* __hoistTriviaOuter(co) {
  ...
  yield* ___hoistTrivia(co, firstTag);
}

function* ___hoistTrivia(co, firstTag) {
  let frame = null;
  let lastFrame = null;
    ...
    if (tag.type === OpenNodeTag) {
      ...
      if (frame?.ref.type === '#' && !frame.ref.name) {
        ...
        if (heldProperties.length) {
          ...
          let node = yield* ___treeFromStream(
            new StreamIterable(___hoistTrivia(co, tag)),
            freeze({}),
          );
          ...
          if (node.value.type === Symbol.for('_')) {
            ...
            for (let property of Tags.traverse(node.value.children)) {
              if (property.value.reference.type === '_') {
                ...
                frame.heldProperties[heldProperties.length - 1] = buildProperty(
                  BList.fromValues(
                    [heldProperty.tags[1][0], heldProperty.tags[1][1] || null, getRoot(node)],
                    1,
                  ),
                );
              } else {
            ...
            frame.heldProperties[heldProperties.length - 1] = buildProperty(
              BList.fromValues([heldProperty.tags[1][0], heldProperty.tags[1][1] || null, node], 1),
            );
          }
      ...
      if (heldProperties.length) {
        ...
        if (!node) {
          heldProperties[heldProperties.length - 1] = buildProperty(
            BList.fromValues(
              [tags[1][0] || null, BList.push(printTag(tag), tags[1][1] || null)],
              1,
            ),
          );
...
function* __generateCSTML(tags, options) {
  ...
  let prevTag = null;
  let iter = getStreamIterator(prettyGroupTags(hoistTrivia(tags), { porcelain: pcl }));
  let step;
  let indentLevel = 0;
  let inline = false;
  let ref = null;
  ...
  for (;;) {
    ...
    if (!inline) {
      ...
      if (!pcl || (tag.type === ReferenceTag && prevTag.type === NullTag)) {
        yield* ' ';
      }
    }

    yield* pcl ? printTag(tag, printOptions) : tag_;
