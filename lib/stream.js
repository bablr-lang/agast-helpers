...
import { buildOpenNodeTag, buildReference, buildReferenceTag } from './builders.js';
import {
  OpenNodeTag,
  CloseNodeTag,
  ReferenceTag,
...
import { cont } from '@bablr/pattern-engine/symbols';
  ...
  for (;;) {
    ...
    if (tag.type === OpenNodeTag) {
      ...
      if (!frame?.ref) {
        ...
        frame = buildEmptyFrame();
        if (isCover) {
          frame.ref = parentFrame?.ref || buildReferenceTag('.');
        }
      }
      ...
      if (tag.value.type === '#' && depth === 1 && parentFrame.open && !wrappedFragment) {
        yield parentFrame.open;
        // parentFrame.ref = null;
        parentFrame.open = null;
