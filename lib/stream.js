...
function* emitHeld(frame) {
  // if (frame.parent?.ref.flags.expression) return;
    ...
    if (tag.type === OpenNodeTag) {
      ...
      if (frame?.ref.type === '#' && !frame.ref.name) {
        ...
        if (heldProperties.length) {
          ...
          if (frame.expanded || frame?.parent?.open.value.type !== Symbol.for('_')) {
            yield* emitHeld(frame);
          }
