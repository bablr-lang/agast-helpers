...
import { printSums, printTag } from './print.js';
import {
  buildChild,
  buildGapTag,
  buildNullTag,
...
import { arrayLast, freeze, freezeRecord, isPlainObject, arrayValues, isRecord } from './object.js';
import { evaluateReturn, maybeWait, stringFromStream } from './iterable.js';
import { buildParser } from './parse.js';
import { parseStreamTag, parseTag, parseTagType } from './parsers.js';
...
function* __streamFromTree(doctypeTag, rootNode, options) {
  ...
  do {
    ...
    do {
      ...
      if (tagPath.type === CloseNodeTag && sums) {
        yield printSums(tagPath.node.value.children[2]);
      }
...
function* emitHeld(frame, nextTag) {
  ...
  for (let property of frame.heldProperties) {
    let { reference, bindings, hash, node } = property;

    let refTag = printTag(buildChild(ReferenceTag, reference));
    if (refTag !== '.:') yield refTag;

    if (!isAnonymousToken(node)) {
      for (let binding of arrayValues(bindings)) {
        yield printTag(buildChild(BindingTag, binding));
      }
    }
    if (hash) {
      yield printTag(buildChild(IdTag), hash);
    }
  ...
  for (;;) {
    ...
    if (tag.type === OpenNodeTag) {
      ...
      if (frame?.ref.type === '#' && !frame.ref.name) {
        ...
        if (heldProperties.length) {
          ...
          if (node.value.type === Symbol.for('_')) {
            ...
            for (let property of Tags.traverse(node.value.children)) {
              if (property.value.reference.type === '_') {
                ...
                frame.heldProperties[heldProperties.length - 1] = buildProperty(
                  BList.fromValues(
                    [heldProperty.tags[1][0], heldProperty.tags[1][1] || '', '', getRoot(node)],
                    1,
                  ),
                );
              } else {
            ...
            frame.heldProperties[heldProperties.length - 1] = buildProperty(
              BList.fromValues(
                [heldProperty.tags[1][0], heldProperty.tags[1][1] || '', '', node],
                1,
              ),
            );
          }
        ...
        if (!frame.shifting && ((frame.ref && frame.ref.type !== '_') || frame.expanded)) {
          ...
          if (!isAnonymous) {
            ...
            if (frame.hash) {
              yield printTag(buildChild(IdTag, frame.hash));
            }
          }
        }
      }
      ...
      if (heldProperties.length) {
        ...
        if (!node) {
          heldProperties[heldProperties.length - 1] = buildProperty(
            BList.fromValues(
              [tags[1][0] || '', BList.push(printTag(tag), tags[1][1] || BList.empty())],
              1,
            ),
          );
      ...
      if (heldProperties.length) {
        ...
        if (!node) {
          heldProperties[heldProperties.length - 1] = buildProperty(
            BList.fromValues([tags[1][0] || '', tags[1][1] || '', tag], 1),
          );
...
function* __generateCSTML(tags, options) {
  ...
  for (;;) {
    ...
    if (pcl && !ref && (tag.type === BindingTag || tag.type === IdTag || Tags.startsNode(tag))) {
      yield '.:';
    }
