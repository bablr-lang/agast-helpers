...
function* __generatePrettyCSTML(tags, options) {
  ...
  for (;;) {
    ...
    if (tag.type === DoctypeTag) {
      if (tag.value.attributes.bablrLanguage) {
        languages.push(tag.value.attributes.bablrLanguage);
      }
    }
    ...
    if (tag.type === TokenGroup) {
      ...
      if (openTag.value.language === languages[languages.length - 1]) {
        let { flags, type, attributes } = openTag.value;
        openTag = buildOpenNodeTag(flags, null, type, attributes);
      }
      yield* printSelfClosingNodeTag(openTag, intrinsicValue);
    } else if (tag.type === OpenNodeTag && tag.value.language === languages[languages.length - 1]) {
    ...
    if (tag.type === OpenNodeTag) {
      indentLevel++;
      languages.push(tag.value.language ?? languages[languages.length - 1]);
    }
